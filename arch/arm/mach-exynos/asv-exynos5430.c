...
#include <mach/asv-exynos5430-fhd.h>
#else
...
#define FS_ABB_CONTROL_REG		(S5P_VA_CHIPID2 + 0x28)
...
static struct asv_reference asv_ref_info = {0, 0, false, false};
...
static void exynos5430_set_asv_info_arm(struct asv_info *asv_inform, bool show_value)
{
	...
	if (asv_ref_info.asv_version == ASV_VER_050) {
		for (i = 0; i < asv_inform->dvfs_level_nr; i++) {
			...
			if (asv_ref_info.is_fs_abb)
				asv_inform->asv_abb[i].asv_value = egl_fused_info.fs_abb;
			else
...
static void exynos5430_set_asv_info_kfc(struct asv_info *asv_inform, bool show_value)
{
	...
	if (asv_ref_info.asv_version == ASV_VER_050) {
		for (i = 0; i < asv_inform->dvfs_level_nr; i++) {
			...
			if (asv_ref_info.is_fs_abb)
				asv_inform->asv_abb[i].asv_value = kfc_fused_info.fs_abb;
			else
...
static void exynos5430_set_asv_info_int(struct asv_info *asv_inform, bool show_value)
{
	...
	if (asv_ref_info.asv_version == ASV_VER_050) {
		for (i = 0; i < asv_inform->dvfs_level_nr; i++) {
			...
			if (asv_ref_info.is_fs_abb)
				asv_inform->asv_abb[i].asv_value = int_fused_info.fs_abb;
			else
...
static void exynos5430_set_asv_info_mif(struct asv_info *asv_inform, bool show_value)
{
	...
	if (asv_ref_info.asv_version == ASV_VER_050) {
		for (i = 0; i < asv_inform->dvfs_level_nr; i++) {
			...
			if (asv_ref_info.is_fs_abb)
				asv_inform->asv_abb[i].asv_value = mif_fused_info.fs_abb;
			else
...
static void exynos5430_set_asv_info_g3d(struct asv_info *asv_inform, bool show_value)
{
	...
	if (asv_ref_info.asv_version == ASV_VER_050) {
		for (i = 0; i < asv_inform->dvfs_level_nr; i++) {
			...
			if (asv_ref_info.is_fs_abb)
				asv_inform->asv_abb[i].asv_value = g3d_fused_info.fs_abb;
			else
...
int exynos5430_init_asv(struct asv_common *asv_info)
{
	...
	fs_abb_con_reg = readl(FS_ABB_CONTROL_REG);
	...
	asv_ref_info.is_fs_abb = (fs_abb_con_reg >> FS_ABB_MASTER_BIT_OFFSET) & FS_ABB_MASTER_BIT_MASK;

	pr_info("EXYNOS5430 ASV : FS_ABB is %s (%d)\n",
			asv_ref_info.is_fs_abb ? "enabled" : "disabled", asv_ref_info.is_fs_abb);

	if (asv_ref_info.is_fs_abb) {
		egl_fused_info.fs_abb = (fs_abb_con_reg >> FS_ABB_EGL_OFFSET) & FS_ABB_MASK;
		egl_fused_info.fs_abb = (1 << 31) | (1 << 7) | (egl_fused_info.fs_abb);
		kfc_fused_info.fs_abb = (fs_abb_con_reg >> FS_ABB_KFC_OFFSET) & FS_ABB_MASK;
		kfc_fused_info.fs_abb = (1 << 31) | (1 << 7) | (kfc_fused_info.fs_abb);
		g3d_fused_info.fs_abb = (fs_abb_con_reg >> FS_ABB_G3D_OFFSET) & FS_ABB_MASK;
		g3d_fused_info.fs_abb = (1 << 31) | (1 << 7) | (g3d_fused_info.fs_abb);
		mif_fused_info.fs_abb = (fs_abb_con_reg >> FS_ABB_MIF_OFFSET) & FS_ABB_MASK;
		mif_fused_info.fs_abb = (1 << 31) | (1 << 7) | (mif_fused_info.fs_abb);
		int_fused_info.fs_abb = (fs_abb_con_reg >> FS_ABB_INT_OFFSET) & FS_ABB_MASK;
		int_fused_info.fs_abb = (1 << 31) | (1 << 7) | (int_fused_info.fs_abb);

		pr_info("EXYNOS5430 ASV : FS_ABB : EGL(0x%08x), KFC(0x%08x), G3D(0x%08x), MIF(0x%08x), INT(0x%08x)\n",
				egl_fused_info.fs_abb, kfc_fused_info.fs_abb, g3d_fused_info.fs_abb,
				mif_fused_info.fs_abb, int_fused_info.fs_abb);
	}
...
#ifdef CONFIG_ASV_MARGIN_TEST
static int __init exynos5430_asv_apply_vol_offset(void)
{
	struct device_node *of_node;
	const char *subtype;
	bool apply_offset = false;

	of_node = of_find_compatible_node(NULL, NULL, "samsung,exynos5430");
	if (!of_node) {
		pr_err("%s: failed to get device node\n", __func__);
		return 0;
	}

	if (of_property_read_string(of_node, "model_info-subtype", &subtype)) {
		pr_err("%s : failed to get model_info-subtype\n", __func__);
		return 0;
	}
	...
	if (apply_offset) {
		 ...
		 * increase vdd_arm by 100mV
		 * increase vdd_kfc by 25mV
		 * increase vdd_mif by 50mV
		 */
		set_arm_volt = 100000;
		set_kfc_volt = 25000;
		set_mif_volt = 50000;

		pr_info("%s: Apply voltage offset: arm(%d), kfc(%d), mif(%d)\n",
				__func__, set_arm_volt, set_kfc_volt,
				set_mif_volt);
	}
...
core_initcall(exynos5430_asv_apply_vol_offset);
#endif /* CONFIG_ASV_MARGIN_TEST */
