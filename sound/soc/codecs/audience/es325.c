...
unsigned int es325_mclk_cnt = 0;

void es325_audio_set_mclk(bool enable, bool forced)
{
	pr_info("%s: %s es325 forced %d cnt %d\n", __func__,enable ? "on" : "off", forced,es325_mclk_cnt);

	/* forced disable */
	if (forced && !enable) {
		pr_info("%s: es325 mclk disbled\n", __func__);
		es325_mclk_cnt = 0;
		writel(0x1001, EXYNOS_PMU_DEBUG);

		return;
	}
	if (enable) {
		if (es325_mclk_cnt == 0) {
			pr_info("%s: es325 mclk enabled\n", __func__);
    	...
    	if (--es325_mclk_cnt > 0)
		{
			pr_info("%s: --es325_mclk_cnt > 0\n", __func__);
			return;
		}
		writel(0x1001, EXYNOS_PMU_DEBUG);
		pr_info("%s: es325 mclk disabled\n", __func__);
	}
...
static int es325_send_cmd(struct es325_data *es325, u32 command, u16 *response)
{
	...
	if (response)
		dev_dbg(es325->dev, "%s(): command=0x%08x response=0x%04x\n", __func__, command, (int)response);
	else
		dev_dbg(es325->dev, "%s(): command=0x%08x\n", __func__, command);
	return ret;
}
...
static void es325_parse_dt(struct device *dev,struct es325_platform_data *pdata)
{
	...
	pdata->gpio_wakeup = of_get_named_gpio(np, "es325-wakeup-gpio",0);
	if(pdata->gpio_wakeup < 0){
		dev_err(dev, "unable to get gpio_wakeup\n");
	}
	pdata->gpio_reset = of_get_named_gpio(np, "es325-reset-gpio",0);
	if(pdata->gpio_reset < 0){
		dev_err(dev, "unable to get gpio_reset\n");
	}

	if (of_property_read_u32_array(np, "es325-pass1", buffer, 2) == 0) {
		pdata->pass[0].src = buffer[0];
		pdata->pass[0].dst = buffer[1];
		dev_info(dev, "es325-pass1 src %d, dst %d\n", pdata->pass[0].src, pdata->pass[0].dst);
	}
	if (of_property_read_u32_array(np, "es325-pass2", buffer, 2) == 0) {
		pdata->pass[1].src = buffer[0];
		pdata->pass[1].dst = buffer[1];
		dev_info(dev, "es325-pass2 src %d, dst %d\n", pdata->pass[1].src, pdata->pass[1].dst);
	}
	if (of_property_read_u32_array(np, "es325-veqparm", buffer, 14) == 0) {

		pdata->veq_parm_table = kzalloc(sizeof(struct es325_veq_parm)*7, GFP_KERNEL);
		if (!pdata) {
			dev_err(dev,"es325_platform_data: alloc fail\n");
			ret = -ENOMEM;
		}

		for( i=0; i< 7; i++)
		{
			pdata->veq_parm_table[i].noise_estimate_adj = buffer[i*2];
			pdata->veq_parm_table[i].max_gain = buffer[i*2+1];
			dev_info(dev, "%d es325-veqparm noise_estimate_adj %d, max_gain %d\n", i+1, pdata->veq_parm_table[i].noise_estimate_adj, pdata->veq_parm_table[i].max_gain);
		}
	}
	if (of_property_read_u32_array(np, "es325-passthrough_src", buffer, 1) == 0) {
		pdata->passthrough_src = buffer[0];
		dev_info(dev, "es325-passthrough_src %d\n", pdata->passthrough_src);
	}
	if (of_property_read_u32_array(np, "es325-passthrough_dst", buffer, 1) == 0) {
		pdata->passthrough_dst = buffer[0];
		dev_info(dev, "es325-passthrough_dst %d\n", pdata->passthrough_dst);
	}
	if (of_property_read_u32_array(np, "es325-use_sleep", buffer, 1) == 0) {
		pdata->use_sleep = buffer[0];
		dev_info(dev, "es325-use_sleep %d\n", pdata->use_sleep);
	}

#ifdef CONFIG_SND_CODEC_CONTROL_MCLK
   pdata->clk_enable = es325_audio_set_mclk;
#else
   pdata->clk_enable = exynos5_audio_set_mclk;
#endif
				...
				const struct i2c_device_id *id)
{
	...
	pdata = devm_kzalloc(&client->dev, sizeof(struct es325_platform_data), GFP_KERNEL);
	if (!pdata) {
		dev_err(es325->dev,"es325_platform_data: alloc fail\n");
		ret = -ENOMEM;
		goto err_kzalloc;
	}
	es325->client = client;
	...
	es325_parse_dt(&client->dev,pdata);
	es325->pdata = pdata;
...
static struct of_device_id es325_match_table[] = {
...
MODULE_DEVICE_TABLE(i2c, es325_id);

static struct i2c_driver es325_driver = {
	.driver = {
		.name	= "audience_es325",
		.owner	= THIS_MODULE,
		.of_match_table = es325_match_table,
	},
	.probe		= es325_probe,
