...
struct clearwater_priv {
	...
	struct clearwater_compr compr_info;
...
SOC_VALUE_ENUM("IN6 OSR", clearwater_in_dmic_osr[5]),
...
static const struct soc_enum clearwater_dsp_output_enum =
	SOC_ENUM_SINGLE(0, 0, ARRAY_SIZE(clearwater_dsp_output_texts),
			clearwater_dsp_output_texts);

static const struct snd_kcontrol_new clearwater_dsp_output_mux[] = {
	SOC_DAPM_ENUM_VIRT("DSP Virtual Output Mux", clearwater_dsp_output_enum),
};
...
static const struct soc_enum clearwater_memory_enum =
	SOC_ENUM_SINGLE(0, 0, ARRAY_SIZE(clearwater_memory_mux_texts),
			clearwater_memory_mux_texts);

static const struct snd_kcontrol_new clearwater_memory_mux[] = {
	SOC_DAPM_ENUM_VIRT("DSP2 Virtual Input", clearwater_memory_enum),
	SOC_DAPM_ENUM_VIRT("DSP3 Virtual Input", clearwater_memory_enum),
};
...
#define CLEARWATER_RATES SNDRV_PCM_RATE_8000_192000
...
static int clearwater_open(struct snd_compr_stream *stream)
{
	...
	if (strcmp(rtd->codec_dai->name, "clearwater-dsp-voicectrl") == 0) {
		n_adsp = 5;
	} else if (strcmp(rtd->codec_dai->name, "clearwater-dsp-trace") == 0) {
		n_adsp = 0;
	} else {
		dev_err(arizona->dev,
	...
	if (!wm_adsp_compress_supported(&clearwater->core.adsp[n_adsp], stream)) {
		dev_err(arizona->dev,
...
static int clearwater_probe(struct platform_device *pdev)
{
	...
	mutex_init(&clearwater->compr_info.lock);
...
error:
	mutex_destroy(&clearwater->compr_info.lock);
