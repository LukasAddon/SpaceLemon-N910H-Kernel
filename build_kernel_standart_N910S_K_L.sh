...
BK=build_kernel
sed -i 's/\-Extended\-/\-Standart\-/g' arch/arm/configs/exynos5433-trelteskt_defconfig
...
GETVER=`grep 'SpaceLemon-Battery-Standart-v.*' arch/arm/configs/exynos5433-trelteskt_defconfig | sed 's/.*-.//g' | sed 's/".*//g'`
###################################### DT.IMG GENERATION #####################################
echo -n "Build dt.img......................................."
...
cp -f arch/arm/boot/zImage build_kernel/AiK-N910S/split_img/boot.img-zImage
cp -f ./dt.img build_kernel/AiK-N910S/split_img/boot.img-dtb

build_kernel/AiK-N910S/repackimg.sh

rm -f build_kernel/out/*.zip

cp -f build_kernel/AiK-N910S/image-new.img build_kernel/out/boot.img

cd build_kernel/out/

zip -r N910S-K-L-SpaceLemon-v${GETVER}-standart.zip ./

cd ../../

rm -f build_kernel/out-no-root/*.zip

cp -f build_kernel/AiK-N910S/image-new.img build_kernel/out-no-root/boot.img
...
zip -r N910S-K-L-SpaceLemon-v${GETVER}-standart-no-root.zip ./
