...
static int sensor_5e3_power_setpin(struct device *dev)
{
	...
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, 0, gpio_none, 0, "VT_CAM_2.8V", 0, PIN_REGULATOR_ON);
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, 1, gpio_none, 0, "VT_CAM_1.2V", 1000, PIN_REGULATOR_ON);
	...
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_OFF, 0, gpio_none, 0, "off", 0, PIN_FUNCTION);
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_OFF, 1, gpio_reset, 0, NULL, 0, PIN_OUTPUT_LOW);
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_OFF, 2, gpio_none, 0, "VT_CAM_1.8V", 0, PIN_REGULATOR_OFF);
	...
	const struct i2c_device_id *id)
{
	...
	device = &core->sensor[SENSOR_S5K5E3_INSTANCE];
...
#ifdef CONFIG_OF
	module->power_setpin = sensor_5e3_power_setpin;
#endif
	ext = &module->ext;
...
static int sensor_5e3_remove(struct i2c_client *client)
{
	int ret = 0;
	return ret;
}
...
static const struct i2c_device_id sensor_5e3_idt[] = {
	{ SENSOR_NAME, 0 },
};

static struct i2c_driver sensor_5e3_driver = {
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
#ifdef CONFIG_OF
	...
	.id_table = sensor_5e3_idt
};

static int __init sensor_5e3_load(void)
{
        return i2c_add_driver(&sensor_5e3_driver);
}

static void __exit sensor_5e3_unload(void)
{
        i2c_del_driver(&sensor_5e3_driver);
}

module_init(sensor_5e3_load);
module_exit(sensor_5e3_unload);
