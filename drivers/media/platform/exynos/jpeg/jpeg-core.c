				...
				int fault_flags, void *p)
{
	...
	dev_err(jpeg->dev, "JPEG SYSMMU Fault is triggered\n");
	dev_err(jpeg->dev, "JPEG dumping registers\n");
	print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 16, 4,
			jpeg->regs, 0x68, false);
	dev_err(jpeg->dev, "JPEG Q-Table\n");
	print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 16, 4,
			jpeg->regs + 0x100, 0x100, false);
	dev_err(jpeg->dev, "JPEG Huffman Table\n");
	print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 16, 4,
...
static int jpeg_clk_get(struct jpeg_dev *jpeg)
{
	...
	for (i = 0; i < JPEG_CLK_NUM; i++) {
		...
		if (IS_ERR(jpeg->clocks[i])) {
			dev_info(dev, "%s clock is not present\n",
				jpeg_clock_names[i]);
			return PTR_ERR(jpeg->clocks[i]);
		}
	}

	if (jpeg->clocks[JPEG_GATE_CLK])
		clk_prepare(jpeg->clocks[JPEG_GATE_CLK]);
...
static int jpeg_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(jpeg->oneshot_dev)) {
		...
		jpeg_clk_put(jpeg);
		return PTR_ERR(jpeg->oneshot_dev);
	}
