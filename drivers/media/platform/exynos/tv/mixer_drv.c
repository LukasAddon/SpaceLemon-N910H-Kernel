...
#ifdef CONFIG_OF
static const struct of_device_id mixer_device_table[] = {
	...
	struct platform_device *pdev)
{
	...
	mdev->res.mxr_regs = devm_request_and_ioremap(mdev->dev, res);
	...
	mdev->res.vp_regs =  = devm_request_and_ioremap(mdev->dev, res);
	...
	ret = devm_request_irq(mdev->dev, res->start, mxr_irq_handler,
				0, "s5p-mixer", mdev);
	if (ret) {
...
static int mxr_probe(struct platform_device *pdev)
{
	...
	struct s5p_mxr_platdata *pdata = NULL;
	struct mxr_device *mdev;
	int ret;
	...
	mdev = devm_kzalloc(dev, sizeof(struct mxr_device), GFP_KERNEL);
	if (!mdev) {
	...
	mdev->pdata = devm_kzalloc(&pdev->dev, sizeof(*pdata), GFP_KERNEL);
	if (!mdev->pdata) {
	...
	if (dev->of_node) {
		of_property_read_u32(dev->of_node, "ip_ver", &mdev->pdata->ip_ver);
		pdata = mdev->pdata;
	} else {
...
static struct platform_driver mxr_driver __refdata = {
	...
	.driver = {
		...
		.owner = THIS_MODULE,
		.pm = &mxr_pm_ops,
		.of_match_table = of_match_ptr(mixer_device_table),
	}
};
