...
#ifdef CONFIG_MFD_RT5033_RESET_WA
	if (info->desc.id == RT5033_ID_LDO1 || info->desc.id ==  RT5033_ID_DCDC1 ) {
		rt_rc = rt5033_regulator_get_status(rdev);
		if((rt_rc == 2) || (rt_rc == 8))
			BUG_ON(1);
	}
#endif
...
static int rt5033_regulator_probe(struct platform_device *pdev)
{
	...
	msleep(1); /* delay 100 us to wait for normal read (from e-fuse) */
