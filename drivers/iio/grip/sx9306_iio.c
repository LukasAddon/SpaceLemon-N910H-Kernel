...
#define SX9306_GRIP_SENSOR_NAME					"sx9306_grip"
...
enum {
	SX9306_SCAN_GRIP_CH,
	SX9306_SCAN_GRIP_TIMESTAMP,
};

#define SX9306_GRIP_INFO_SHARED_MASK	(BIT(IIO_CHAN_INFO_SCALE))
#define SX9306_GRIP_INFO_SEPARATE_MASK	(BIT(IIO_CHAN_INFO_RAW))

static const struct iio_chan_spec sx9306_channels[] = {
    {
	.type = IIO_GRIP,
	.modified = 1,
	.channel2 = IIO_MOD_GRIP,
	.info_mask_separate = SX9306_GRIP_INFO_SEPARATE_MASK,
	.info_mask_shared_by_type = SX9306_GRIP_INFO_SHARED_MASK,
	.scan_index = SX9306_SCAN_GRIP_CH,
	.scan_type = IIO_ST('s', 32, 32, 0),
    },
    IIO_CHAN_SOFT_TIMESTAMP(SX9306_SCAN_GRIP_TIMESTAMP),
};
...
static inline s64 sx9306_iio_get_boottime_ns(void)
{
	struct timespec ts;
...
static int sx9306_grip_data_rdy_trig_poll(struct iio_dev *indio_dev)
{
	struct sx9306_p *st = iio_priv(indio_dev);
	unsigned long flags;

	spin_lock_irqsave(&st->spin_lock, flags);
	iio_trigger_poll(st->trig, sx9306_iio_get_boottime_ns());
	spin_unlock_irqrestore(&st->spin_lock, flags);
	return 0;
}

static void send_event(struct sx9306_p *data, int cnt, u8 state)
{
	u8 buf;
	...
	if (state == ACTIVE) {
		...
		pr_info("[SX9306]: %s - %d button touched\n", __func__, cnt);
	} else {
		data->touchMode = NORMAL_TOUCH_MODE;
		data->state[cnt] = IDLE;
		sx9306_i2c_write(data, SX9306_CPS_CTRL6_REG, buf);
		pr_info("[SX9306]: %s - %d button released\n", __func__, cnt);
	}
	...
	case 0:
		...
		if (state == ACTIVE) {
			...
			sx9306_grip_data_rdy_trig_poll(indio_dev);
        	} else
			data->iio_state = IDLE;
			sx9306_grip_data_rdy_trig_poll(indio_dev);
        break;
	...
	case 2:
	case 3:
		pr_info("[SX9306]: %s - There is no defined event for" \
			" button %d.\n", __func__, cnt);
		break;
	default:
		break;
...
static void sx9306_display_data_reg(struct sx9306_p *data)
{
	...
	for (i = 0; i < TOTAL_BOTTON_COUNT; i++) {
		sx9306_i2c_write(data, SX9306_REGSENSORSELECT, i);
		pr_info("[SX9306]: ############# %d button #############\n", i);
		for (reg = SX9306_REGUSEMSB;
			reg <= SX9306_REGOFFSETLSB; reg++) {
...
static s32 sx9306_get_capMain(struct sx9306_p *data)
{
	u8 msByte = 0;
	u8 lsByte = 0;
	u16 offset = 0;
	...
	sx9306_i2c_write(data, SX9306_REGSENSORSELECT, REF_SENSOR);
	sx9306_i2c_read(data, SX9306_REGUSEMSB, &msByte);
	sx9306_i2c_read(data, SX9306_REGUSELSB, &lsByte);
#endif
	...
	pr_info("[SX9306]: %s - CapsMain: %ld, useful: %ld, Offset: %u\n",
...
static void sx9306_touchCheckWithRefSensor(struct sx9306_p *data)
{
	...
	capMain = sx9306_get_capMain(data);
...
static int sx9306_get_useful(struct sx9306_p *data)
{
	u8 msByte = 0;
	u8 lsByte = 0;
	s16 fullByte = 0;
	...
	pr_info("[SX9306]: %s - PROXIUSEFUL = %d\n", __func__, fullByte);

	return (int)fullByte;
}

static int sx9306_get_offset(struct sx9306_p *data)
{
	u8 msByte = 0;
	u8 lsByte = 0;
	u16 fullByte = 0;
	...
	pr_info("[SX9306]: %s - PROXIOFFSET = %u\n", __func__, fullByte);
...
static int sx9306_do_calibrate(struct sx9306_p *data, bool do_calib)
{
	...
	data->calData[2] = sx9306_get_offset(data);
	...
	data->calData[1] = sx9306_get_useful(data);
		...
		struct device_attribute *attr, char *buf)
{
	...
	sx9306_i2c_write(data, SX9306_REGSENSORSELECT, MAIN_SENSOR);
	sx9306_i2c_read(data, SX9306_REGUSEMSB, &msb);
	sx9306_i2c_read(data, SX9306_REGUSELSB, &lsb);
	useful = (s16)((msb << 8) | lsb);

	sx9306_i2c_read(data, SX9306_REGOFFSETMSB, &msb);
	sx9306_i2c_read(data, SX9306_REGOFFSETLSB, &lsb);
...
static void sx9306_touch_process(struct sx9306_p *data)
{
	u8 status = 0;
	int cnt;
	s32 capMain, threshold;
	...
	for (cnt = 0; cnt < TOTAL_BOTTON_COUNT; cnt++) {
		if (data->state[cnt] == IDLE) {
			if (status & (CSX_STATUS_REG << cnt)) {
				...
				pr_info("[SX9306]: %s - %d already released.\n",
					__func__, cnt);
			}
			...
			if (!(status & (CSX_STATUS_REG << cnt))) {
				...
				pr_info("[SX9306]: %s - %d still touched\n",
					__func__, cnt);
			}
		}
	}
...
static void sx9306_process_interrupt(struct sx9306_p *data)
{
	...
	status = sx9306_read_irqstate(data);

	if (status & IRQ_PROCESS_CONDITION)
		sx9306_touch_process(data);
}
...
static irqreturn_t sx9306_adjdet_interrupt_thread(int irq, void *pdata)
{
	...
	struct sx9306_p *data = iio_priv(indio_dev);
...
irqreturn_t sx9306_iio_pollfunc_store_boottime(int irq, void *p)
{
	struct iio_poll_func *pf = p;
	pf->timestamp = sx9306_iio_get_boottime_ns();
	return IRQ_WAKE_THREAD;
}


static irqreturn_t sx9306_trigger_handler(int irq, void *p)
{
	struct iio_poll_func *pf = p;
...
static int sx9306_pseudo_irq_enable(struct iio_dev *indio_dev)
{
	struct sx9306_p *st = iio_priv(indio_dev);
	if (!atomic_cmpxchg(&st->pseudo_irq_enable, 0, 1)) {
		mutex_lock(&st->lock);
		sx9306_set_enable(st, 1);
		mutex_unlock(&st->lock);
	}
	return 0;
}

static int sx9306_pseudo_irq_disable(struct iio_dev *indio_dev)
{
	struct sx9306_p *st = iio_priv(indio_dev);
	if (atomic_cmpxchg(&st->pseudo_irq_enable, 1, 0)) {
		mutex_lock(&st->lock);
		sx9306_set_enable(st, 0);
		mutex_unlock(&st->lock);
	}
	return 0;
}

static int sx9306_set_pseudo_irq(struct iio_dev *indio_dev, int enable)
{
	if (enable)
		sx9306_pseudo_irq_enable(indio_dev);
	else
		sx9306_pseudo_irq_disable(indio_dev);
	return 0;
}

static int sx9306_data_rdy_trigger_set_state(struct iio_trigger *trig,
		bool state)
{
	struct iio_dev *indio_dev = iio_trigger_get_drvdata(trig);
	sx9306_set_pseudo_irq(indio_dev, state);
	return 0;
}

static const struct iio_trigger_ops sx9306_trigger_ops = {
	.owner = THIS_MODULE,
	.set_trigger_state = &sx9306_data_rdy_trigger_set_state,
};

static int sx9306_probe_trigger(struct iio_dev *indio_dev)
{
	int ret;
	struct sx9306_p *st = iio_priv(indio_dev);
	indio_dev->pollfunc = iio_alloc_pollfunc(&sx9306_iio_pollfunc_store_boottime,
			&sx9306_trigger_handler, IRQF_ONESHOT, indio_dev,
	...
	st->trig->ops = &sx9306_trigger_ops;
...
static void sx9306_remove_trigger(struct iio_dev *indio_dev)
{
	struct sx9306_p *st = iio_priv(indio_dev);
...
static const struct iio_buffer_setup_ops sx9306_buffer_setup_ops = {
	.preenable = &iio_sw_buffer_preenable,
	.postenable = &iio_triggered_buffer_postenable,
	.predisable = &iio_triggered_buffer_predisable,
};

static int sx9306_probe_buffer(struct iio_dev *indio_dev)
{
	int ret;
	...
	indio_dev->setup_ops = &sx9306_buffer_setup_ops;
	...
	iio_scan_mask_set(indio_dev, indio_dev->buffer, SX9306_SCAN_GRIP_CH);
...
static void sx9306_remove_buffer(struct iio_dev *indio_dev)
{
	iio_buffer_unregister(indio_dev);
	iio_kfifo_free(indio_dev->buffer);
};

static int sx9306_read_raw(struct iio_dev *indio_dev,
...
static const struct iio_info sx9306_info = {
	.read_raw = &sx9306_read_raw,
		...
		const struct i2c_device_id *id)
{
	...
	struct sx9306_p *data_input = NULL;
	...
	data_input = kzalloc(sizeof(struct sx9306_p), GFP_KERNEL);
	if (data_input == NULL) {
	...
	indio_dev->name = SX9306_GRIP_SENSOR_NAME;
	indio_dev->dev.parent = &client->dev;
	indio_dev->info = &sx9306_info;
	indio_dev->channels = sx9306_channels;
	indio_dev->num_channels = ARRAY_SIZE(sx9306_channels);
	...
	ret = request_threaded_irq(data->irq, NULL, sx9306_interrupt_thread,
			IRQF_TRIGGER_FALLING|IRQF_ONESHOT , "sx9306_irq", data);
	if (ret < 0) {
	...
	ret = sx9306_probe_buffer(indio_dev);
	if (ret)
		goto exit_iio_buffer_probe_failed;
	ret = sx9306_probe_trigger(indio_dev);
	if (ret)
	...
	ret = sx9306_input_init(data_input);
	if (ret < 0)
...
exit_iio_register_failed:
	sx9306_remove_trigger(indio_dev);
exit_iio_trigger_probe_failed:
	sx9306_remove_buffer(indio_dev);
...
//	sysfs_remove_group(&indio_dev->dev.kobj, &sx9306_attribute_group);
	mutex_destroy(&data->mode_mutex);
