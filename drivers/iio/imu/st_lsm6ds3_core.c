...
static struct st_lsm6ds3_odr_table {
	...
	.odr_avl[3] = { .hz = 100,
			...
			/* to limit fusion odr */
			.ns = (unsigned long)(NSEC_PER_SEC/100)},
	.odr_avl[4] = { .hz = 208,
...
EXPORT_SYMBOL(st_lsm6ds3_acc_open_calibration);
...
int st_lsm6ds3_gyro_open_calibration(struct lsm6ds3_data *cdata)
{
	...
	pr_info("%s open gyro calibration %d, %d, %d\n", __func__,
		cdata->gyro_cal_data[0], cdata->gyro_cal_data[1],
		cdata->gyro_cal_data[2]);

	return ret;
}
EXPORT_SYMBOL(st_lsm6ds3_gyro_open_calibration);

static int st_lsm6ds3_gyro_save_calibration(struct lsm6ds3_data *cdata,
		s32 cal_data[3])
{
	int ret = 0;
	mm_segment_t old_fs;
	...
	cdata->gyro_cal_data[0] = cal_data[0];
	cdata->gyro_cal_data[1] = cal_data[1];
	cdata->gyro_cal_data[2] = cal_data[2];
	...
	ret = cal_filp->f_op->write(cal_filp,
			(char *)&cdata->gyro_cal_data, 3 * sizeof(s16),
			&cal_filp->f_pos);
	if (ret != 3 * sizeof(s16)) {
	...
	pr_info("%s: %d, %d, %d\n", __func__, cdata->gyro_cal_data[0],
		cdata->gyro_cal_data[1], cdata->gyro_cal_data[2]);
	...
	if (!((sdata->cdata->sensors_enabled &
		...
		if (enable) {
			err = st_lsm6ds3_write_data_with_mask(sdata->cdata,
				...
				st_lsm6ds3_odr_table.odr_avl[1].value);
			if (err < 0)
				return err;
		} else {
			err = st_lsm6ds3_write_data_with_mask(sdata->cdata,
...
static int st_lsm6ds3_enable_sensors(struct lsm6ds3_sensor_data *sdata)
{
	...
	case ST_INDIO_DEV_ACCEL:
		...
		err = st_lsm6ds3_write_data_with_mask(sdata->cdata,
				...
				st_lsm6ds3_odr_table.odr_avl[4].value);
		if (err < 0)
			return err;
	...
	case ST_INDIO_DEV_GYRO:
		...
		if (sdata->sindex == ST_INDIO_DEV_GYRO) {
			if (sdata->cdata->skip_gyro_data == false) {
				sdata->cdata->skip_gyro_data = true;
				sdata->cdata->skip_gyro_cnt = 0;
				dev_info(sdata->cdata->dev, "%s skip_gyro on\n",
						__func__);
			}
		}
		break;
...
static int st_lsm6ds3_disable_sensors(struct lsm6ds3_sensor_data *sdata)
{
	...
	case ST_INDIO_DEV_GYRO:
		if (sdata->cdata->skip_gyro_data == true) {
			...
			sdata->cdata->skip_gyro_cnt = 0;
			dev_info(sdata->cdata->dev, "%s skip_gyro off\n",
							__func__);
		}
		err = st_lsm6ds3_write_data_with_mask(sdata->cdata,
				st_lsm6ds3_odr_table.addr[sdata->sindex],
	...
	dev_info(sdata->cdata->dev, "%s sensors_disabled %d (%x)...\n",
		__func__, sdata->sindex, sdata->cdata->sensors_enabled);
...
void st_lsm6ds3_reg_dump(struct lsm6ds3_data *cdata)
{
	...
	err = st_lsm6ds3_reg_read(cdata, ST_LSM6DS3_STEP_COUNTER_EN_ADDR, 1, regs);
	if (err < 0) {
		dev_err(cdata->dev, "failed to read TAP_CFG registers.\n");
	}
	for (i = 0; i < 1; i++) {
		dev_info(cdata->dev, "TAP CFG [%d]: %x\n", i, regs[i]);
	}
}
...
static int st_lsm6ds3_init_sensor(struct lsm6ds3_data *cdata)
{
	...
	cdata->step_c.last_step_timestamp = st_lsm6ds3_iio_get_boottime_ns();
...
static int st_lsm6ds3_gyr_fifo_test(struct lsm6ds3_sensor_data *sdata,
	s16 *zero_rate_lsb, s32 *fifo_cnt, u8 *fifo_pass, s16 *slot_raw, char *out_str)
{
	int err;
	u8 buf[5] = {0x00,};
		...
		*fifo_pass = 1;
		...
		for (i = 0; i < 3; i++) {
			if (ABS(zero_rate_delta[i]) > ST_LSM6DS3_GYR_ZRL_DELTA) {
				pr_info("%s, zero_rate_delta fail : [%d] %d\n",
...
static int st_lsm6ds3_selftest_run(struct lsm6ds3_sensor_data *sdata,	int test, char *out_str)
{
	...
	zero[8] = 0x38;
	zero[9] = 0x38;
	err = st_lsm6ds3_reg_write(sdata->cdata, ST_LSM6DS3_CTRL1_ADDR, 10, zero);
	if (err < 0) {
		dev_err(sdata->cdata->dev, "failed to write 0 into CTRL registers.\n");
		goto restore_regs;
	}
	...
	case ST_INDIO_DEV_GYRO:
		...
		if (test & 2) {
			...
			cal_pass = st_lsm6ds3_gyro_save_calibration(sdata->cdata, NOST);
				...
				struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if ((enable == 1) && (cdata->sa_flag == 0)) {
		...
		pr_info("%s - smart alert is off! irq = %d, odr 0x%x\n",
			__func__, cdata->sa_irq_state, val >> 4);
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = st_lsm6ds3_i2c_read_dev(cdata, ST_LSM6DS3_OUT_TEMP_L_ADDR, data, 2);
	if (ret < 0)
...
static int st_lsm6ds3_parse_dt(struct lsm6ds3_data *cdata)
{
	...
	if (of_property_read_u32_array(np,
		"st,orientation", orientation, 9) < 0) {
...
int st_lsm6ds3_common_suspend(struct lsm6ds3_data *cdata)
{
	...
	pr_info("%s called(sensor:%u,sa:%d)\n", __func__,
		tmp_sensors_enabled, cdata->sa_flag);
...
#if (LSM6DS3_HRTIMER_TRIGGER > 0)
		if (sdata->hr_timer_en > 0)
			hrtimer_start(&sdata->hr_timer,
				sdata->ktime, HRTIMER_MODE_REL);
#endif
	}

	pr_info("%s called(sensor:%u,sa:%d)\n", __func__,
		cdata->sensors_enabled, cdata->sa_flag);
