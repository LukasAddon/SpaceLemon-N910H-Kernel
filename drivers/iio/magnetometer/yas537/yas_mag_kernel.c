...
static int yas_device_read(int32_t type, uint8_t addr, uint8_t *buf, int len)
{
	...
	if (err != 2) {
		pr_err("[SENSOR]: %s error addr: %u, err: %d\n",
			__func__, addr, err);
		return err;
	}
	return 0;
...
static irqreturn_t yas_trigger_handler(int irq, void *p)
{
	...
	if (((pf->timestamp - st->old_timestamp) >
		(s64)st->poll_delay * 1800000LL) && (st->old_timestamp != 0)) {

		if (indio_dev->scan_timestamp)
			*(s64 *)((u8 *)mag + ALIGN(len, sizeof(s64))) =
				(pf->timestamp + st->old_timestamp) >> 1;
		iio_push_to_buffers(indio_dev, (u8 *)mag);
		printk("[SENSOR] %s - gap : %lld\n",
			__func__, pf->timestamp - st->old_timestamp);
	}
			...
			struct  yas_state *st)
{
	...
	if (!of_property_read_u32(np, "yas,orientation",
				&st->position))
		pr_info("[SENSOR]: %s - yas,orientation = %d\n",
			__func__, st->position);
	else
		return -EINVAL;

	if (of_property_read_u32_array(np,
		"yas,softiron", softiron, 9) < 0) {
		pr_err("[SENSOR: %s - get softiron(%d) error\n",
			__func__, softiron[0]);
	}

	if (of_property_read_u32_array(np,
		"yas,softiron_sign", sign, 9) < 0) {
		pr_err("[SENSOR]: %s - get softiron(%d) error\n",
			__func__, sign[0]);
	}

	for (i = 0 ; i < 9 ; i++){
		st->m[i] = (softiron[i])*(sign[i]-1);
		pr_info("[SENSOR]: %s - softiron = %d, sign = %d\n",
			__func__, softiron[i], sign[i]);
	}
...
static int yas_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	st->client = i2c;
	st->sampling_frequency = 5;
	st->poll_delay = MSEC_PER_SEC / st->sampling_frequency;
	...
	ret = sensors_register(factory_dev, indio_dev, mag_sensor_attrs,
		MODULE_NAME_MAG);
	if (ret < 0) {
		pr_err("[SENSOR]: %s- cound not register mag sensor(%d).\n",
			__func__, ret);
