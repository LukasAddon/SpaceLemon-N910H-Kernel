...
static int bcm4773_spi_probe( struct spi_device *spi )
{
	...
	s3c_gpio_cfgpin (g_bport->mcu_req, S3C_GPIO_OUTPUT);
	s3c_gpio_setpull(g_bport->mcu_req, S3C_GPIO_PULL_DOWN);
#endif

	// MCU_REQ_RESP	GPIO4 : EXYNOS_GPX3[3]
	gpio_request(g_bport->mcu_resp, "MCU RESP");
	gpio_direction_input(g_bport->mcu_resp);
#ifdef CONFIG_MACH_UNIVERSAL5420
	s3c_gpio_cfgpin (g_bport->mcu_resp, S3C_GPIO_INPUT);
	bcm4773_mcu_resp_float(false); 	//s3c_gpio_setpull(EXYNOS5420_GPX3(3), S3C_GPIO_PULL_UP);
#endif
#endif
	return 0;
}
