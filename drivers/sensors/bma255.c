...
static int bma255_parse_dt(struct bma255_p *data, struct device *dev)
{
	...
	data->acc_int1 = of_get_named_gpio_flags(np, "accelerometer,int1_gpio", 0, &flags);
	if (data->acc_int1 < 0) {
		pr_err("[SENSOR]: %s - get acc_int1 error\n", __func__);
		return -ENODEV;
	}
	data->sda_gpio = of_get_named_gpio_flags(np, "accelerometer,sda-gpio", 0, &flags);
	if (data->sda_gpio < 0)
		pr_info("[SENSOR]: %s - no sda_gpio\n", __func__);

	data->scl_gpio = of_get_named_gpio_flags(np, "accelerometer,scl-gpio", 0, &flags);
	if (data->scl_gpio < 0)
...
#else
#define bma255_match_table	NULL
#endif
...
static struct i2c_driver bma255_driver = {
	.driver = {
		...
		.pm = &bma255_pm_ops,
#ifdef CONFIG_OF
	.of_match_table = of_match_ptr(bma255_match_table),
#endif
	},
	.probe		= bma255_probe,
