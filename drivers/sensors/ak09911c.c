...
static int ak09911c_parse_dt(struct ak09911c_p *data, struct device *dev)
{
	...
	if (of_property_read_u32(np, "magnetic_sensor,chip_pos", &data->chip_pos) < 0)
		data->chip_pos = AK09911C_BOTTOM_LOWER_RIGHT;

	data->m_rst_n = of_get_named_gpio_flags(np, "magnetic_sensor,m_rst_n", 0, &flags);
	if (data->m_rst_n < 0){
...
#else
#define ak09911c_match_table	NULL
#endif
...
static struct i2c_driver ak09911c_driver = {
	.driver = {
		...
		.pm = &ak09911c_pm_ops,
#ifdef CONFIG_OF
		.of_match_table = of_match_ptr(ak09911c_match_table),
#endif
	},
	.probe		= ak09911c_probe,
	.shutdown	= ak09911c_shutdown,
	.remove		= __devexit_p(ak09911c_remove),
