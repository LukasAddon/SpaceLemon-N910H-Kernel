...
#ifdef CONFIG_OF
static int cm3223_parse_dt(struct cm3323_p *data, struct device *dev)
{
	if (data == NULL)
		return -ENODEV;
	return 0;
...
#else
static int cm3223_parse_dt(struct bma255_p *data, struct device *dev)
{
	return -ENODEV;
}
#endif
		...
		const struct i2c_device_id *id)
{
	...
	ret = cm3223_parse_dt(data, &client->dev);
	if (ret < 0) {
