...
#endif
static int tmd3782_regulator_onoff(struct taos_data *data, int onoff)
{
	int err;

	data->vdd_3p3 = regulator_get(NULL, "GES_LED_3P3");
...
static int taos_chip_on(struct taos_data *taos)
{
	...
	tmd3782_regulator_onoff(taos,ON);
...
static int taos_chip_off(struct taos_data *taos)
{
	...
	tmd3782_regulator_onoff(taos,OFF);
				      ...
				      const char *buf, size_t size)
{
	...
	if (new_value && !(taos->power_state & PROXIMITY_ENABLED)) {
		...
		mdelay(10);
		printk(KERN_ERR "%s, Thes_high = %d,Thes_low = %d\n",\
			__func__,taos_proximity_get_threshold(taos, PRX_MAXTHRESHLO),taos_proximity_get_threshold(taos, PRX_MINTHRESHLO));
...
static void taos_work_func_prox(struct work_struct *work)
{
	...
	if (ret < 0)
		gprintk("opt_i2c_read failed, err = %d, i= %d\n", ret,i);
...
//	int irq;
	...
	taos->irq = gpio_to_irq(pdata->als_int);

	rc = request_threaded_irq(taos->irq, NULL,
			 taos_irq_handler,
			...
			struct  taos_platform_data *pdata)
{
	...
	// set threshold - prox_rawdata_trim value
	pdata->prox_thresh_hi = 650,//800
	pdata->prox_thresh_low = 470,//620
	pdata->prox_th_hi_cal = 380,//530
	pdata->prox_th_low_cal = 250,//400
	//set offset- prox_rawdata_trim value
	pdata->crosstalk_max_offset = 350,//500
...
sreboot:
	...
	ret=sensors_register(taos->proximity_dev, taos, prox_sensor_attrs,MODULE_NAME_PROX);
	 if (ret < 0) {
