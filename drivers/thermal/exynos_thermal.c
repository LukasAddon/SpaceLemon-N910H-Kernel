...
#ifdef CONFIG_ARM_EXYNOS_MP_CPUFREQ
		if (cluster_idx == CA7) {
			cpufreq_get_policy(&policy, CA7_POLICY_CORE);

			if (clip_data->freq_clip_max_kfc > policy.max) {
				pr_warn("%s: throttling freq(%d) is greater than policy max(%d)\n", __func__, clip_data->freq_clip_max_kfc, policy.max);
				clip_data->freq_clip_max_kfc = policy.max;
			} else if (clip_data->freq_clip_max_kfc < policy.min) {
				pr_warn("%s: throttling freq(%d) is less than policy min(%d)\n", __func__, clip_data->freq_clip_max_kfc, policy.min);
		...
		unsigned long event, void *v)
{
	...
	if (old_state != mif_thermal_state) {
		switch (mif_thermal_state) {
		case MIF_NORMAL:
			pm_qos_update_request(&exynos_mif_thermal_big_max_qos, PM_QOS_CPU_FREQ_DEFAULT_VALUE);
			pm_qos_update_request(&exynos_mif_thermal_little_max_qos, PM_QOS_CPU_FREQ_DEFAULT_VALUE);
			break;
		case MIF_THROTTLING1:
			pm_qos_update_request(&exynos_mif_thermal_big_max_qos, MIF_THROTTLING1_BIG);
			pm_qos_update_request(&exynos_mif_thermal_little_max_qos, MIF_THROTTLING1_LITTLE);
			break;
		case MIF_THROTTLING2:
		case MIF_TRIPPING:
			pm_qos_update_request(&exynos_mif_thermal_big_max_qos, MIF_THROTTLING2_BIG);
			pm_qos_update_request(&exynos_mif_thermal_little_max_qos, MIF_THROTTLING2_LITTLE);
			break;
		}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (soc_is_exynos5422()) {
		...
		/* Exynos5430/Exynos5433: Sensor type of each sub-block
		 * 0: EAGLE0, 1: EAGLE1, 2: G3D, 3: KFC, 4: ISP
		 */
		for (i = 0; i < EXYNOS_TMU_COUNT; i++)
			len += sprintf(&buf[len], "%lu,", temp[i]);
