...
int enable_display_driver_power(struct device *dev)
{
	...
	if (num_supplies) {
		...
		ret = regulator_bulk_enable(num_supplies, supplies);
	...
	if (IS_ERR(pinctrl)) {
		pr_err("failed to get default pinctrl\n");
...
int disable_display_driver_power(struct device *dev)
{
	...
	if (num_supplies) {
		...
		regulator_bulk_disable(num_supplies, supplies);
		regulator_bulk_free(num_supplies, supplies);
	}
...
int decon_vclk_eclk_mux_control(bool enable)
{
	...
	/* Temporary patch to avoid mux_set_parent fail log */
	if (MAX_CLK_GATING_COUNT == 0)
		return ret;
