...
static int s3c_fb_set_par(struct fb_info *info)
{
	...
	/* write WnBPPORDER register to set rgb order */
...
static int s3c_fb_win_map_on(struct s3c_fb *sfb, int color)
{
	int i;
	u32 color_map = WINxMAP_MAP | color;

	for (i = 0; i < sfb->variant.nr_windows; i++)
		writel(color_map, sfb->regs + WINxMAP(i));
...
#if defined(CONFIG_FB_I80_COMMAND_MODE) && defined(CONFIG_LCD_PCD)
	if ((!sfb->output_on && sfb->power_state == POWER_DOWN) || sfb->pcd_detected)
#else
...
static void pcd_detection_enable(struct s3c_fb *sfb)
{
	static unsigned int pcd_enable_skip;
	int pcd;

	if (unlikely(!pcd_enable_skip)) {
		pcd_enable_skip = 1;
		return;
}
...
static irqreturn_t pcd_detection_isr(int irq, void *_sfb)
{
	...
	pcd_det_level = gpio_get_value(sfb->pcd);
...
static int s3c_fb_enable(struct s3c_fb *sfb)
{
	...
	disp_pm_runtime_get_sync(get_display_driver());
