...
struct pinctrl_state	*state_turnon_tes;
struct pinctrl_state	*state_turnoff_tes;
#endif
...
void set_default_hibernation_mode(struct display_driver *dispdrv)
{
	...
	if (lpcharge)
		...
		power_gating = true;
#endif
#if defined(CONFIG_FB_HIBERNATION_DISPLAY_POWER_GATING_DEEPSTOP) && !defined(CONFIG_SEC_FACTORY)
		hotplug_gating = true;
#endif
	}
