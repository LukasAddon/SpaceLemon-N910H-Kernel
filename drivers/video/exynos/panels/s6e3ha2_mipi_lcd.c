/* drivers/video/decon_display/s6e3ha2_mipi_lcd.c
 *
...
static int esd_fg_init(struct lcd_info *lcd)
{
	...
	if(lcd->oled_det_gpio == -ENOENT) {
		dev_err(&lcd->ld->dev, "failed to get oled-det-gpio number\n");
		return -EINVAL;
	}
...
#if defined(CONFIG_DECON_MDNIE_LITE)
static int s6e3ha2_send_seq(struct lcd_info *lcd, struct lcd_seq_info *seq, u32 num)
{
	int ret = 0;

	mutex_lock(&lcd->bl_lock);
	ret = s6e3ha2_write_set(lcd, seq, num);
...
static int s6e3ha2_set_fps(struct lcd_info *lcd)
{
	int ret = 0;
	s6e3ha2_write_set(lcd, SEQ_LDI_FPS_SET, ARRAY_SIZE(SEQ_LDI_FPS_SET));
	return ret;
}
...
static int s6e3ha2_set_partial_display(struct lcd_info *lcd)
{
	if (lcd->partial_range[0] || lcd->partial_range[1]) {
		s6e3ha2_write(lcd, SEQ_PARTIAL_AREA, ARRAY_SIZE(SEQ_PARTIAL_AREA));
		s6e3ha2_write(lcd, SEQ_PARTIAL_MODE_ON, ARRAY_SIZE(SEQ_PARTIAL_MODE_ON));
	} else
		s6e3ha2_write(lcd, SEQ_NORMAL_MODE_ON, ARRAY_SIZE(SEQ_NORMAL_MODE_ON));
...
static int s6e3ha2_ldi_init(struct lcd_info *lcd)
{
	...
	/* 9. Interface Setting */
	s6e3ha2_write(lcd, SEQ_TEST_KEY_ON_F0, ARRAY_SIZE(SEQ_TEST_KEY_ON_F0));
	s6e3ha2_write(lcd, SEQ_SINGLE_DSI_SET1, ARRAY_SIZE(SEQ_SINGLE_DSI_SET1));
	s6e3ha2_write(lcd, SEQ_SINGLE_DSI_SET2, ARRAY_SIZE(SEQ_SINGLE_DSI_SET2));
	/* Calibratino code */
	...
	if (lcd->id[1] == 0)
		s6e3ha2_write(lcd, SEQ_TOUCH_HSYNC_ON_ID2_00, ARRAY_SIZE(SEQ_TOUCH_HSYNC_ON_ID2_00));
	else
		s6e3ha2_write(lcd, SEQ_TOUCH_HSYNC_ON, ARRAY_SIZE(SEQ_TOUCH_HSYNC_ON));
	s6e3ha2_write(lcd, SEQ_PENTILE_CONTROL, ARRAY_SIZE(SEQ_PENTILE_CONTROL));

	s6e3ha2_write(lcd, SEQ_POC_GLOBAL, ARRAY_SIZE(SEQ_POC_GLOBAL));
	s6e3ha2_write(lcd, SEQ_POC_SETTING, ARRAY_SIZE(SEQ_POC_SETTING));
...
static int s6e3ha2_check_panel(struct lcd_info *lcd)
{
	int ret = 0;
	u8 Booster_volt;

	ret = s6e3ha2_read(lcd, LDI_RDDPM_REG, &Booster_volt, 1);
...
#endif
		s6e3ha2_write(lcd, SEQ_TEST_KEY_ON_F0, ARRAY_SIZE(SEQ_TEST_KEY_ON_F0));
		ret = s6e3ha2_read(lcd, LDI_ESDERR_REG, &Booster_volt, 1);
...
#ifdef CONFIG_LCD_ALPM
static int s6e3ha2_ldi_alpm_init(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e3ha2_ldi_alpm_exit(struct lcd_info *lcd)
{
	int ret = 0;

	s6e3ha2_write_set(lcd, SEQ_ALPM_OFF_SET, ARRAY_SIZE(SEQ_ALPM_OFF_SET));
...
static const unsigned int HMT_DIM_TABLE[HMT_IBRIGHTNESS_MAX] = {
	...
	68,	72,	77,	82,	87,	93,	99,	105, HMT_MAX_GAMMA,	360
};
...
static int hmt_on_set(struct lcd_info *lcd, u8 forced)
{
	...
	if (forced || (lcd->current_hmt_on != lcd->hmt_on)) {
		if (lcd->hmt_on) {
			s6e3ha2_write_set(lcd, SEQ_HMT_SINGLE_SET, ARRAY_SIZE(SEQ_HMT_SINGLE_SET));
			s6e3ha2_write_set(lcd, SEQ_HMT_REVERSE_SET, ARRAY_SIZE(SEQ_HMT_REVERSE_SET));
		} else {
			s6e3ha2_write_set(lcd, SEQ_HMT_OFF_SET, ARRAY_SIZE(SEQ_HMT_OFF_SET));
			s6e3ha2_write_set(lcd, SEQ_HMT_AID_FORWARD_SET, ARRAY_SIZE(SEQ_HMT_AID_FORWARD_SET));
		}
	}
...
{
	s6e3ha2_write(lcd, lcd->hmt_gamma_table[lcd->hmt_bl], GAMMA_PARAM_SIZE);
...
aid_update:
	...
	s6e3ha2_write(lcd, hmt_aor, ARRAY_SIZE(hmt_aor));
...
elvss_update:
	if(lcd->hmt_brightness == HMT_MAX_GAMMA) {
		...
		buf[2] = 0x00;
		s6e3ha2_write(lcd, buf, ARRAY_SIZE(buf));
	}
	else
 		s6e3ha2_write(lcd, HMT_ELVSS_TABLE[lcd->acl_enable], ARRAY_SIZE(HMT_ELVSS_TABLE[lcd->acl_enable]));
 	lcd->hmt_current_acl = lcd->acl_enable;
	return 0;
}

static int hmt_set_vint(struct lcd_info *lcd, u8 forced)
{
	if(forced)
		goto vint_update;
	return 0;
vint_update:
	s6e3ha2_write(lcd, SEQ_VINT_SETTING, ARRAY_SIZE(SEQ_VINT_SETTING));
	return 0;
}
...
static int hmt_read_hbmgamma(struct lcd_info *lcd, u8 *buf)
{
	...
	ret = s6e3ha2_read(lcd, LDI_HBMGAMMA_REG, buf, LDI_HBMGAMMA_LEN);
...
#if defined(CONFIG_DECON_MDNIE_LITE)
	mdnie_register(&lcd->ld->dev, lcd, (mdnie_w)s6e3ha2_send_seq, (mdnie_r)s6e3ha2_read, &tune_info);
#endif
