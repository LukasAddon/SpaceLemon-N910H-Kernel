...
#define MDNIE_SYSFS_PREFIX		"/sdcard/mdnie_ext/"
#define PANEL_COORDINATE_PATH	"/sys/class/lcd/panel_ext/color_coordinate"
...
static int mdnie_write(struct mdnie_info *mdnie, struct mdnie_table *table)
{
	...
	if (mdnie->enable)
		ret = mdnie->ops.write(mdnie->data, table->seq, MDNIE_CMD_MAX);
...
static void mdnie_update(struct mdnie_info *mdnie)
{
	...
	if (!IS_ERR_OR_NULL(table) && !IS_ERR_OR_NULL(table->name)) {
		...
		mdnie->white_r = table->seq[MDNIE_CMD1].cmd[MDNIE_WHITE_R];
		mdnie->white_g = table->seq[MDNIE_CMD1].cmd[MDNIE_WHITE_G];
		mdnie->white_b = table->seq[MDNIE_CMD1].cmd[MDNIE_WHITE_B];
	}
...
static void update_color_position(struct mdnie_info *mdnie, unsigned int idx)
{
	...
	for (mode = 0; mode < MODE_MAX; mode++) {
		for (scenario = 0; scenario <= EMAIL_MODE; scenario++) {
			wbuf = main_table[scenario][mode].seq[MDNIE_CMD1].cmd;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret < 0)
		...
		if (value == COLOR_BLIND) {
			...
			wbuf = &accessibility_table[COLOR_BLIND].seq[MDNIE_CMD1].cmd[MDNIE_COLOR_BLIND_OFFSET];
		...
		struct device_attribute *attr, char *buf)
{
	...
	/* mdnie->ops.write(mdnie->data, table->seq[LEVEL_KEY_UNLOCK].cmd, table->seq[LEVEL_KEY_UNLOCK].len); */
	...
	/* mdnie->ops.write(mdnie->data, table->seq[LEVEL_KEY_LOCK].cmd, table->seq[LEVEL_KEY_LOCK].len); */
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (mdnie->enable && (mdnie->accessibility == ACCESSIBILITY_OFF)
		...
		memcpy(mdnie->sequence_buffer,
			table->seq[MDNIE_CMD1].cmd,
			table->seq[MDNIE_CMD1].len);
		mdnie->table_buffer.seq[MDNIE_CMD1].cmd
				 ...
				 unsigned long event, void *data)
{
	...
	dev_info(mdnie->dev, "%s: %d, %d\n", __func__, fb_blank, evdata->info->node);

	if (evdata->info->node != 2)
		return 0;
...
int mdnie2_register(struct device *p, void *data, mdnie_w w, mdnie_r r)
{
	int ret = 0;
	struct mdnie_info *mdnie;

	mdnie_class = class_create(THIS_MODULE, "mdnie_ext");
	if (IS_ERR_OR_NULL(mdnie_class)) {
		pr_err("failed to create mdnie_ext class\n");
		ret = -EINVAL;
		goto error0;
	}
	...
	if (!mdnie) {
		pr_err("failed to allocate mdnie_ext\n");
		ret = -ENOMEM;
		goto error1;
	}

	mdnie->dev = device_create(mdnie_class, p, 0, &mdnie, "mdnie_ext");
	if (IS_ERR_OR_NULL(mdnie->dev)) {
		pr_err("failed to create mdnie_ext device\n");
		ret = -EINVAL;
		goto error2;
	}
