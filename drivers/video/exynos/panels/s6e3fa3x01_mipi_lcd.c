/* drivers/video/decon_display/s6e3fa3x01_mipi_lcd.c
 *
...
#include "s6e3fa3x01_param.h"
#include "dynamic_aid_s6e3fa3x01.h"
#include <linux/syscalls.h>

#if defined(CONFIG_DECON_MDNIE_LITE)
#include "mdnie.h"
#include "mdnie_lite_table_a8.h"
#endif
...
#endif
#if defined(CONFIG_LCD_ELON1_DET)
	unsigned int	elon1_det_gpio;
#endif

	struct mipi_dsim_device		*dsim;
};

static int s6e3fa3x01_write(struct lcd_info *lcd, const u8 *seq, u32 len)
{
	int ret = 0;
	int retry;
...
#if defined(PRINT_CMD)
	printk("%s:: REG[0x%02x] ",__func__, seq[0]);
...
static int s6e3fa3x01_read(struct lcd_info *lcd, u8 addr, u8 *buf, u32 len)
{
	int ret = 0;
	u8 cmd;
	int retry;
...
static int s6e3fa3x01_write_set(struct lcd_info *lcd, struct lcd_seq_info *seq, u32 num)
{
	int ret = 0, i;

	for (i = 0; i < num ; i++) {
		if (seq[i].cmd) {
			ret = s6e3fa3x01_write(lcd, seq[i].cmd, seq[i].len);
			if (ret != 0) {
...
#if defined(CONFIG_DECON_MDNIE_LITE)
static int s6e3fa3x01_send_seq(struct lcd_info *lcd, struct lcd_seq_info *seq, u32 num)
{
	int ret = 0;

	mutex_lock(&lcd->bl_lock);
	ret = s6e3fa3x01_write_set(lcd, seq, num);
...
static void s6e3fa3x01_read_coordinate(struct lcd_info *lcd)
{
	int ret;
	unsigned char buf[LDI_COORDINATE_LEN] = {0,};

	ret = s6e3fa3x01_read(lcd, LDI_COORDINATE_REG, buf, LDI_COORDINATE_LEN);

	if (ret < 1)
		dev_err(&lcd->ld->dev, "%s failed\n", __func__);
	lcd->mdnie_code[0] = buf[0];
	lcd->mdnie_code[1] = buf[1];
	lcd->mdnie_code[2] = buf[2];
	lcd->mdnie_code[3] = buf[3];

	lcd->coordinate[0] = buf[0] << 8 | buf[1];	/* X */
	lcd->coordinate[1] = buf[2] << 8 | buf[3];	/* Y */
}

static void s6e3fa3x01_read_id(struct lcd_info *lcd, u8 *buf)
{
	int ret;

	ret = s6e3fa3x01_read(lcd, LDI_ID_REG, buf, LDI_ID_LEN);
	if (ret < 1) {
		lcd->connected = 0;
...
static void s6e3fa3x01_read_ddi_id(struct lcd_info *lcd, u8 *buf)
{
	int ret;

	ret = s6e3fa3x01_read(lcd, LDI_ID2_REG, buf, LDI_ID2_LEN);

	if (ret < 1)
		dev_info(&lcd->ld->dev, "%s failed\n", __func__);
}

static int s6e3fa3x01_read_mtp(struct lcd_info *lcd, u8 *buf)
{
	int ret, i;

	ret = s6e3fa3x01_read(lcd, LDI_MTP_REG, buf, LDI_MTP_LEN);
	...
	lcd->date[4] = buf[44];	// seconds
	lcd->date[5] = buf[45];	// frame1
	lcd->date[6] = buf[46];	// frame2
...
static int s6e3fa3x01_read_elvss(struct lcd_info *lcd, u8 *buf)
{
	int ret, i;

	ret = s6e3fa3x01_read(lcd, LDI_ELVSS_REG, buf, LDI_ELVSS_LEN);
...
static int s6e3fa3x01_read_tset(struct lcd_info *lcd)
{
	int ret, i;

	lcd->tset_table[0] = LDI_TSET_REG;

	ret = s6e3fa3x01_read(lcd, LDI_TSET_REG, &lcd->tset_table[1], LDI_TSET_LEN);
...
static int s6e3fa3x01_gamma_ctl(struct lcd_info *lcd)
{
	s6e3fa3x01_write(lcd, lcd->gamma_table[lcd->bl], GAMMA_PARAM_SIZE);

	return 0;
}

static int s6e3fa3x01_aid_parameter_ctl(struct lcd_info *lcd, u8 force)
{
	if (force)
		goto aid_update;
...
aid_update:
	s6e3fa3x01_write(lcd, lcd->aor[lcd->bl], AID_PARAM_SIZE);
...
static int s6e3fa3x01_set_elvss(struct lcd_info *lcd, u8 force)
{
	int ret = 0, i;
	u32 elvss_level, nit;
...
elvss_update:
	...
	lcd->elvss[22] = elvss.hbm;

	ret = s6e3fa3x01_write(lcd, lcd->elvss, ELVSS_PARAM_SIZE);
...
static int s6e3fa3x01_set_vint(struct lcd_info *lcd, u8 force)
{
	int ret = 0, i, level;
	u32 nit;
...
static int s6e3fa3x01_set_tset(struct lcd_info *lcd, u8 force)
{
	int ret = 0;
	u8 tset;
	...
	if (force || lcd->temperature != lcd->current_temp) {
		...
		ret = s6e3fa3x01_write(lcd, lcd->tset_table, LDI_TSET_LEN + 1);
...
static int s6e3fa3x01_set_hbm(struct lcd_info *lcd, u8 force)
{
	int ret = 0, level = LEVEL_IS_HBM(lcd->auto_brightness);

	if (force || lcd->current_hbm != HBM_TABLE[level][1]) {
		ret = s6e3fa3x01_write(lcd, HBM_TABLE[level], HBM_PARAM_SIZE);
...
static int s6e3fa3x01_set_partial_display(struct lcd_info *lcd)
{
	if (lcd->partial_range[0] || lcd->partial_range[1]) {
		s6e3fa3x01_write(lcd, SEQ_PARTIAL_AREA, ARRAY_SIZE(SEQ_PARTIAL_AREA));
		s6e3fa3x01_write(lcd, SEQ_PARTIAL_DISP, ARRAY_SIZE(SEQ_PARTIAL_DISP));
	} else
		s6e3fa3x01_write(lcd, SEQ_NORMAL_DISP, ARRAY_SIZE(SEQ_NORMAL_DISP));
...
static int init_vint_table(struct lcd_info *lcd)
{
	...
	for (i = 0; i < VINT_STATUS_MAX; i++) {
		...
		memcpy(lcd->vint_table[i], SEQ_VINT_SETTING, ARRAY_SIZE(SEQ_VINT_SETTING));

		lcd->vint_table[i][2] = VINT_TABLE[i];
	}

	for (i = 0; i < VINT_STATUS_MAX; i++) {
		for (j = 0; j < ARRAY_SIZE(SEQ_VINT_SETTING); j++)
...
#if defined(CONFIG_ESD_FG)
static void s6e3fa3x01_err_fg_enable(struct lcd_info *lcd, int enable)
{
...
static int s6e3fa3x01_err_fg_blank(struct lcd_info *lcd)
{
...
static void s6e3fa3x01_err_fg_is_detected(struct lcd_info *lcd)
{
	struct file *fp;
	struct timespec ts;
	struct rtc_time tm;
...
static void s6e3fa3x01_err_fg_work(struct work_struct *work)
{
	struct lcd_info *lcd =
	...
	s6e3fa3x01_err_fg_is_detected(lcd);

	s6e3fa3x01_err_fg_blank(lcd);
}
static irqreturn_t s6e3fa3x01_err_fg_isr(int irq, void *dev_id)
{
...
static int s6e3fa3x01_err_fg_init(struct lcd_info *lcd)
{
	...
	INIT_DELAYED_WORK(&lcd->err_fg, s6e3fa3x01_err_fg_work);

	ret = request_irq(lcd->err_fg_irq, s6e3fa3x01_err_fg_isr, IRQF_TRIGGER_RISING |
...
static int update_brightness(struct lcd_info *lcd, u8 force)
{
	...
	if ((force) || ((lcd->ldi_enable) && (lcd->current_bl != lcd->bl))) {
		...
		s6e3fa3x01_gamma_ctl(lcd);
		s6e3fa3x01_aid_parameter_ctl(lcd, force);
		s6e3fa3x01_set_elvss(lcd, force);

		s6e3fa3x01_set_acl(lcd, force);
		s6e3fa3x01_set_tset(lcd, force);
		s6e3fa3x01_set_hbm(lcd, force);
		s6e3fa3x01_set_vint(lcd, force);
		s6e3fa3x01_write(lcd, SEQ_GAMMA_UPDATE, ARRAY_SIZE(SEQ_GAMMA_UPDATE));
		s6e3fa3x01_write(lcd, SEQ_TEST_KEY_OFF_F0, ARRAY_SIZE(SEQ_TEST_KEY_OFF_F0));
...
static int s6e3fa3x01_ldi_init(struct lcd_info *lcd)
{
	int ret = 0;
	...
	/* Common Setting*/
	s6e3fa3x01_write(lcd, SEQ_TE_ON, ARRAY_SIZE(SEQ_TE_ON));
	s6e3fa3x01_write(lcd, SEQ_GLOB_PARA, ARRAY_SIZE(SEQ_GLOB_PARA));
	s6e3fa3x01_write(lcd, SEQ_AVC_SETTING, ARRAY_SIZE(SEQ_AVC_SETTING));
#if defined(CONFIG_LCD_PCD)
	s6e3fa3x01_write(lcd, SEQ_PCD_SETTING, ARRAY_SIZE(SEQ_PCD_SETTING));	// default high, low active
#endif
#if defined(CONFIG_ESD_FG)
	s6e3fa3x01_write(lcd, SEQ_ERR_FG_SETTING, ARRAY_SIZE(SEQ_ERR_FG_SETTING));
#endif
	/* Brightness Control */
	update_brightness(lcd, 1);

	s6e3fa3x01_write(lcd, SEQ_TEST_KEY_OFF_FC, ARRAY_SIZE(SEQ_TEST_KEY_OFF_FC));

	msleep(120);

	s6e3fa3x01_write(lcd, SEQ_MEM_ACCESS1, ARRAY_SIZE(SEQ_MEM_ACCESS1));
	s6e3fa3x01_write(lcd, SEQ_MEM_ACCESS2, ARRAY_SIZE(SEQ_MEM_ACCESS2));

	return ret;
}

static int s6e3fa3x01_ldi_enable(struct lcd_info *lcd)
{
	int ret = 0;

	s6e3fa3x01_write(lcd, SEQ_DISPLAY_ON, ARRAY_SIZE(SEQ_DISPLAY_ON));
	dev_info(&lcd->ld->dev, "DISPLAY_ON\n");

	return ret;
}

static int s6e3fa3x01_ldi_disable(struct lcd_info *lcd)
{
	int ret = 0;
	...
	/* Display Off (28h) */
	s6e3fa3x01_write(lcd, SEQ_DISPLAY_OFF, ARRAY_SIZE(SEQ_DISPLAY_OFF));
	...
	/* Sleep In (10h) */
	s6e3fa3x01_write(lcd, SEQ_SLEEP_IN, ARRAY_SIZE(SEQ_SLEEP_IN));
...
static int s6e3fa3x01_power_on(struct lcd_info *lcd)
{
	int ret = 0;

	dev_info(&lcd->ld->dev, "+ %s\n", __func__);

	ret = s6e3fa3x01_ldi_init(lcd);
	if (ret) {
		dev_err(&lcd->ld->dev, "failed to initialize ldi.\n");
		goto err;
	}
#if 0
	ret = s6e3fa3x01_ldi_enable(lcd);
	if (ret) {
...
#if defined(CONFIG_ESD_FG)
	s6e3fa3x01_err_fg_enable(lcd, 1);
#endif
...
static int s6e3fa3x01_power_off(struct lcd_info *lcd)
{
	int ret = 0;
...
#if defined(CONFIG_ESD_FG)
	s6e3fa3x01_err_fg_enable(lcd, 0);
#endif
...
#if defined(CONFIG_LCD_ELON1_DET)
static int s6e3fa3x01_get_elon1_det(struct lcd_info *lcd)
{
	struct device *dev = lcd->dev;
	int gpio, elon1;

	gpio = of_get_named_gpio(dev->of_node, "oled-elon1_det", 0);
	if (gpio < 0) {
		dev_err(dev, "failed to get proper gpio number\n");
		return -EINVAL;
	}

	elon1 = gpio_get_value(gpio);

	dev_info(dev, "%s : elon1 %d\n", __func__, elon1);

	return elon1;
}
#endif

static int s6e3fa3x01_power(struct lcd_info *lcd, int power)
{
	int ret = 0;

	if (POWER_IS_ON(power) && !POWER_IS_ON(lcd->power)) {
#if defined(CONFIG_LCD_ELON1_DET)
	if (!s6e3fa3x01_get_elon1_det(lcd)) {
		dev_err(&lcd->ld->dev, "failed to get elon1 signal.\n");
		ret = s6e3fa3x01_power_off(lcd);
	} else
#endif
		ret = s6e3fa3x01_power_on(lcd);
...
static int s6e3fa3x01_set_power(struct lcd_device *ld, int power)
{
	...
	return s6e3fa3x01_power(lcd, power);
}

static int s6e3fa3x01_get_power(struct lcd_device *ld)
{
	struct lcd_info *lcd = lcd_get_data(ld);
...
static int s6e3fa3x01_check_fb(struct lcd_device *ld, struct fb_info *fb)
{
	return 0;
}

static int s6e3fa3x01_get_brightness(struct backlight_device *bd)
{
...
static int s6e3fa3x01_set_brightness(struct backlight_device *bd)
{
	int ret = 0;
...
static struct lcd_ops s6e3fa3x01_lcd_ops = {
	.set_power = s6e3fa3x01_set_power,
	.get_power = s6e3fa3x01_get_power,
	.check_fb  = s6e3fa3x01_check_fb,
};

static const struct backlight_ops s6e3fa3x01_backlight_ops = {
	.get_brightness = s6e3fa3x01_get_brightness,
	.update_status = s6e3fa3x01_set_brightness,
};
	...
	struct device_attribute *attr, char *buf)
{
	...
	sprintf(buf, "%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X\n",
		...
		lcd->date[5],lcd->date[6],lcd->mdnie_code[0],lcd->mdnie_code[1],
		lcd->mdnie_code[2],lcd->mdnie_code[3]);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (lcd->ldi_enable)
		s6e3fa3x01_set_partial_display(lcd);
				 ...
				 unsigned long event, void *data)
{
	...
	if (fb_blank == FB_BLANK_UNBLANK)
		s6e3fa3x01_ldi_enable(lcd);

	return 0;
}

static int lcd_register_fb(struct lcd_info *lcd)
{
	memset(&lcd->fb_notif, 0, sizeof(lcd->fb_notif));
	lcd->fb_notif.notifier_call = fb_notifier_callback;
	return fb_register_client(&lcd->fb_notif);
}

static int s6e3fa3x01_probe(struct mipi_dsim_device *dsim)
{
	int ret;
	struct lcd_info *lcd;
	...
	lcd->ld = lcd_device_register("panel", dsim->dev, lcd, &s6e3fa3x01_lcd_ops);
	if (IS_ERR(lcd->ld)) {
	...
	lcd->bd = backlight_device_register("panel", dsim->dev, lcd, &s6e3fa3x01_backlight_ops, NULL);
	if (IS_ERR(lcd->bd)) {
	...
	s6e3fa3x01_read_elvss(lcd, elvss_data);
	s6e3fa3x01_read_tset(lcd);
	s6e3fa3x01_write(lcd, SEQ_TEST_KEY_OFF_F0, ARRAY_SIZE(SEQ_TEST_KEY_OFF_F0));
...
#if defined(CONFIG_DECON_MDNIE_LITE)
	mdnie_register(&lcd->ld->dev, lcd, (mdnie_w)s6e3fa3x01_send_seq, (mdnie_r)s6e3fa3x01_read, &tune_info);
#endif

#if defined(CONFIG_ESD_FG)
	s6e3fa3x01_err_fg_init(lcd);
#endif
...
static int s6e3fa3x01_displayon(struct mipi_dsim_device *dsim)
{
...
static int s6e3fa3x01_suspend(struct mipi_dsim_device *dsim)
{
	struct lcd_info *lcd = dev_get_drvdata(&dsim->lcd->dev);
...
static int s6e3fa3x01_resume(struct mipi_dsim_device *dsim)
{
	return 0;
}

struct mipi_dsim_lcd_driver s6e3fa3x01_mipi_lcd_driver = {
	.probe		= s6e3fa3x01_probe,
	.displayon	= s6e3fa3x01_displayon,
	.suspend	= s6e3fa3x01_suspend,
	.resume		= s6e3fa3x01_resume,
};

static int s6e3fa3x01_init(void)
{
	return 0;
}

static void s6e3fa3x01_exit(void)
{
	return;
}

module_init(s6e3fa3x01_init);
module_exit(s6e3fa3x01_exit);

MODULE_DESCRIPTION("MIPI-DSI s6e3fa3x01 (1080*1920) Panel Driver");
MODULE_LICENSE("GPL");
