...
struct mipi_dsim_config g_dsim_config = {
	...
	.dsim_ddi_pd = &s6e3ha0k_mipi_lcd_driver,
#elif defined(CONFIG_DECON_LCD_S6E3HA2K)
	...
	.dsim_ddi_pd = &ea8064g_mipi_lcd_driver,
...
static void parse_disp_sysreg(void)
{
	...
	g_disp_sysreg.disp_sysreg[0] = of_iomap(nd, 0);
	if (!g_disp_sysreg.disp_sysreg[0]) {
		pr_info("Failed to get sysreg or there is not related sysreg.\n");
	}
}
...
int parse_lcd_drvdata(struct device_node *np)
{
	...
	if (gpio_is_valid(gpio)) {
		if (!gpio_request(gpio, "OLED_ID")) {
			idx = gpio_get_value(gpio);
			gpio_free(gpio);
		}
	}
#endif
	...
	DT_READ_U32_OPTIONAL(node, "timing,hsync-2h-cycle", g_decon_lcd.hsync_2h_cycle);
	DT_READ_U32_OPTIONAL(node, "type_of_ddi", g_decon_lcd.ddi_type);

	g_decon_lcd.hs_clk *= 1000000;
	g_decon_lcd.esc_clk *= 1000000;
