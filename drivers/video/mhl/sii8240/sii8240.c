 ...
 *	   Kyungrok Min <gyoungrok.min@samsung.com>
...
#define CONFIG_MHL_SWING_LEVEL

static bool check_vbus_present(void);

static int mhl_write_byte_reg(struct i2c_client *client, u32 offset, u8 value)
{
...
static void sii8240_link_monitor_work(struct work_struct *work)
{
	...
	struct sii8240_data *sii8240 = container_of(work, struct sii8240_data,
						mhl_link_monitor_work);
	struct i2c_client *tpi = sii8240->pdata->tpi_client;

	if (sii8240->state < STATE_MHL_DISCOVERY_SUCCESS) {
		sii8240->monitor_cmd |= 0x08;
		sii8240->monitor_cmd &= ~(0x01);
		del_timer_sync(&sii8240->mhl_timer);

		ret = exynos_smc(MC_FC_HDCP_VALUE, 0, sii8240->monitor_cmd, 0);
		if (ret != MC_FC_RET_OK)
			pr_err("exynos_smc fail ret = %d\n", ret);

		pr_info("%s() monitor_cmd = %d, mhl status = %d\n",
			__func__, sii8240->monitor_cmd, sii8240->state);
		return;
	}

	ret = mhl_read_byte_reg(tpi, TPI_HDCP_QUERY_DATA_REG, &hdcp_query);
	if (ret < 0) {
		pr_err("%s() mhl has been shut-down\n", __func__);
		return;
	}
	...
	if (rd_data) {
		...
		pr_debug("%s() repeater extended = %d\n", __func__, status);
	} else {
	...
	if (status == 0) {
		sii8240->monitor_cmd |= 0x04;
		pr_info("%s() monitor_cmd = %d\n",
			__func__, sii8240->monitor_cmd);
	} else
		sii8240->monitor_cmd |= 0x02;

	ret = exynos_smc(MC_FC_HDCP_VALUE, 0, sii8240->monitor_cmd, 0);
	if (ret != MC_FC_RET_OK)
		pr_err("exynos_smc fail ret = %d\n", ret);

	mod_timer(&sii8240->mhl_timer, jiffies + msecs_to_jiffies(2000));
}
#endif
...
static int sii8240_hdcp_on(struct sii8240_data *sii8240, bool hdcp_on)
{
	...
	if (hdcp_on) {
		...
		if (ret < 0)
			pr_warn("unable to set HDCP_CTRL register\n");
		...
		if (ret < 0)
			pr_warn("unable to reset HDCP_CTRL register to 0x00\n");
...
static int sii8240_tmds_active_hdcp(struct sii8240_data *sii8240)
{
	...
	if (ret < 0)
		pr_info("sii8240: set_mute_mode fail %d\n", __LINE__);
...
static int sii8240_hdcp_control(struct sii8240_data *sii8240, u8 hdcp_reg)
{
	...
	if (hdcp_reg & BIT_TPI_INTR_ST0_HDCP_SECURITY_CHANGE_EVENT) {
		...
		case LINK_STATUS_LINK_LOST:
			...
			ret = set_mute_mode(sii8240, false);
			if (ret < 0)
				pr_info("[ERROR]sii8240: set_mute_mode on fail\n");

			ret = sii8240_hdcp_on(sii8240, false);
			if (ret < 0)
				pr_info("[ERROR]sii8240: tmds_control on fail\n");
			ret = sii8240_tmds_active_hdcp(sii8240);
			break;
		...
		case LINK_STATUS_RENEGOTIATION_REQ:
			...
			ret = sii8240_hdcp_on(sii8240, false);
			if (ret < 0)
				pr_info("[ERROR]sii8240: sii8240_hdcp_on off fail\n");
			msleep(500);
			ret = sii8240_tmds_active_hdcp(sii8240);
			if (ret < 0)
				pr_info("[ERROR]sii8240: set_mute_mode on fail\n");
		...
		case LINK_STATUS_LINK_SUSPENDED:
			...
			ret = sii8240_hdcp_on(sii8240, false);
			if (ret < 0)
				pr_info("[ERROR]sii8240: hdcp off fail\n");
			break;
		}
	}
	...
	*or that an Ri mismatch has caused authentication to fail.*/
	if (hdcp_reg & BIT_TPI_INTR_ST0_HDCP_AUTH_STATUS_CHANGE_EVENT) {
		...
		case (EXTENDED_LINK_PROTECTION_NONE |
			...
			ret = sii8240_hdcp_on(sii8240, false);
			if (ret < 0)
				pr_info("[ERROR]sii8240: sii8240_hdcp_on  fail\n");

			ret = sii8240_tmds_active_hdcp(sii8240);
			if (ret < 0)
				pr_info("[ERROR]sii8240: sii8240_tmds_active_hdcp fail\n");
			break;
		...
		case LOCAL_LINK_PROTECTION_SECURE:
			...
			if (!(HDCP_REPEATER_MASK & hdcp_query)) {
				ret = set_mute_mode(sii8240, false);
				if (ret < 0)
					pr_info("[ERROR]sii8240: tmds_control off fail\n");

#ifdef SII8240_CHECK_MONITOR
				if ((sii8240->monitor_cmd & 0x01) == 0x01) {
					/*start checking link status*/
					sii8240_link_monitor_timer
						((unsigned long) sii8240);
					pr_info("monitor start\n");
				} else
		...
		default:
			...
			ret = sii8240_tmds_active_hdcp(sii8240);
			if (ret < 0)
				pr_info("sii8240: tmds_control off fail\n");
			break;
		}
	}
	/*Read BKSV Done.
	*Various bits in 0x29 contain information acquired from BCAPs.*/
	if (hdcp_reg & BIT_TPI_INTR_ST0_BKSV_DONE) {
		...
		if (hdcp_query & PROTECTION_TYPE_MASK) {
			...
			if (ret < 0)
				pr_info("sii8240: %s():sii8240_hdcp_on is fail, %d\n",
				__func__, ret);
		}
	}
	if (hdcp_reg & BIT_TPI_INTR_ST0_BKSV_ERR) {
		...
		ret = mhl_read_block_reg(tmds, 0x10, AKSV_SIZE, bksv);
		if (ret < 0) {
			pr_info("sii8240: %s():%d failed !\n",
			__func__, __LINE__);
			return ret;
		}
		pr_info("sii8240: BKSV :0x%x, 0x%x, 0x%x, 0x%x, 0x%x",
		bksv[0], bksv[1], bksv[2], bksv[3], bksv[4]);

		ret = sii8240_hdcp_on(sii8240, false);
		if (ret < 0)
			pr_warn("sii8240: %s() ii8240_hdcp_on is fail, %d\n",
				__func__, ret);
		ret = sii8240_tmds_active_hdcp(sii8240);
		if (ret < 0)
			pr_info("sii8240: tmds_control off fail\n");
...
static int sii8240_read_edid_block(struct sii8240_data *sii8240, u8 edid_ext)
{
	...
	if (i == 100) {
		pr_info("sii8240: EDID READ timeout\n");
		return -ETIMEDOUT;
	}
...
edid_populated:
	...
	exynos_get_edid(sii8240->edid, (1 + edid_exts)
		* EDID_LENGTH,
...
static int mhl_hpd_control_low(struct sii8240_data *sii8240)
{
	...
	if (ret < 0)
		pr_info("[ERROR]sii8240: %s():%d failed !\n",
		__func__, __LINE__);
...
static int  sii8240_set_interrupt(struct sii8240_data *sii8240)
{
	...
	if (ret < 0) {
		pr_info("[ERROR]sii8240:sii8240:failed !\n");
		return ret;
	}
...
static int sii8240_init_regs(struct sii8240_data *sii8240)
{
	...
	if (sii8240->state == STATE_MHL_CONNECTED) {
		...
		pr_info("sii8240: INIT REGS swing_level: 0x%02X\n",
	...
	if (ret < 0)
		pr_info("sii8240_hdcp_key_check failed !\n");
...
static int sii8240_devcap_read_locked(struct sii8240_data *sii8240, u8 offset)
{
	...
	if (ret < 0)
		pr_info("msc req locked error\n");

	ret = mhl_read_byte_reg(cbus, MSC_RCVD_DATA1_REG, &val);
	if (ret < 0) {
		pr_info("msc rcvd data reg read failing\n");
		return ret;
	}
	return val;
}
...
#if 0 /*unused*/
static void sii8240_unregister_input_device(struct sii8240_data *sii8240)
{
	mutex_lock(&sii8240->input_lock);
	if (sii8240->input_dev) {
		pr_debug("sii8240: unregistering input device\n");
		input_unregister_device(sii8240->input_dev);
		sii8240->input_dev = NULL;
	}
	mutex_unlock(&sii8240->input_lock);
}
#endif
...
static void cbus_process_rap_key(struct sii8240_data *sii8240, u8 key)
{
	...
	case RAP_CONTENT_OFF:
		...
		break;
	default:
		pr_debug("sii8240: unrecognized RAP code %u\n", key);
							...
							u8 offset, u8 data)
{
	...
	if (ret < 0) {
		pr_info("sii8240: msc_req_locked error : 0x%X\n", offset);
		return ret;
	}
...
static void sii8240_setup_charging(struct sii8240_data *sii8240)
{
	...
	if ((peer_devcap[MHL_DEVCAP_MHL_VERSION] & 0xF0) >= 0x20) {
		...
		pow = MHL_FLD_GET(value, 4, 4);
		plim = MHL_FLD_GET(value, 6, 5);
		pr_info("%s: type=%d, pow=%d, plim=%d\n",
						__func__, devType, pow, plim);

		/* PLIM1 and PLIM0 together indicate the minimum VBUS power
		* [PLIM1, PLIM0]
		...
		* {1,1} = Reserved*/
		...
		/*When set, device can output power across VBUS to the device
		* connected through the cable,
		* When clear, device cannot output power
		* across VBUS to the device
		* connected through the cable*/
		if (pow == 1) {
			/*Powered dongle*/
	...
	(struct sii8240_data *sii8240, bool pack_pixel)
{
	...
	if (pack_pixel)
		pr_debug("sii8240: %s(): pack_pixel\n", __func__);
	else
		pr_debug("sii8240: %s(): none pack_pixel\n", __func__);
	if (pack_pixel)
	...
	pr_info("sii8240: colorspace : 0x%X :", colorspace);
	...
	switch (input_range) {
	case 0x00:
		pr_cont("input_range AUTO\n");
		input_range = 0x00;
		break;

	case 0x04:
		pr_cont("input_range Limited\n");
		input_range = 0x08;
		break;

	case 0x08:
		pr_cont("input_range FULL\n");
		input_range = 0x04;
		break;

	case 0x0C:
		pr_cont("input_range Reserved\n");
		input_range = 0x00;
		break;

	default:
		pr_cont("input_range default\n");
		input_range = 0x00;
		break;
	}
...
static int sii8240_packed_pixel_avi_info(struct sii8240_data *sii8240)
{
	...
	if (patternGen) {
		...
		pr_info("sii8240: convert Color space\n");
...
static void sii8240_detection_restart(struct work_struct *work)
{
	...
	if (sii8240_set_mhl_timing(sii8240) < 0) {
		pr_info("sii8240: set mhl timing failed\n");
		goto err_exit;
	}
...
err_exit:
	queue_work(sii8240->cbus_cmd_wqs, &sii8240->redetect_work);
disconnection_exit:
	mutex_unlock(&sii8240->lock);
}
					...
					unsigned long event, void *data)
{
	 ...
	 * external connector
	 */
	sii8240->pdata->hw_onoff(1);
	sii8240->pdata->hw_reset();

	ret = sii8240_init_regs(sii8240);
	if (ret < 0) {
		pr_err("sii8240: sii8240_init_regs error\n");
		goto unhandled;
	}
	...
	if (ret < 0) {
		pr_err("sii8240: switch_to_d3 error!\n");
		goto unhandled;
	}
...
static int sii8240_msc_irq_handler(struct sii8240_data *sii8240, u8 intr)
{
	...
	if (intr & MSC_HPD_RCVD) {
		pr_info("sii8240: Downstream HPD Changed\n");
		...
		if (DOWNSTREAM_HPD_MASK & hpd) {
			pr_info("sii8240: HPD HIGH , Overriden: 0x%X\n", temp);
			...
			pr_info("sii8240: HPD LOW , Overriden: 0x%X\n", temp);
			if (temp & HPD_OVERRIDE_EN) {
	...
	if (intr & MSC_SET_INT_RCVD) {
		...
		if (cbus_intr[1] & MHL_INT_EDID_CHG) {
			pr_info("sii8240: MHL_INT_EDID_CHG\n");
		...
		u8 int1, u8 cbus_con_st)
{
	...
	if ((int1 & BIT_INTR4_MHL_EST) && (sii8240->cbus_connected)) {
		...
		pr_debug("sii8240: schedule_work: cbus_work\n");
		queue_work(sii8240->cbus_cmd_wqs, &sii8240->cbus_work);
...
static void sii8240_avif_check_callback(unsigned long data)
{
	...
	if (((sii8240->aviInfoFrame[0] == 0x82) &&
		...
		schedule_work(&sii8240->avi_check_work);
}
...
static int sii8240_check_scdt_reg(struct sii8240_data *sii8240)
{
	...
	if (sii8240->r281 != 0) {
		pr_info("sii8240:%s r281 = 0x%02x\n", __func__, sii8240->r281);
...
static int sii8240_audio_video_intr_control(struct sii8240_data *sii8240)
{
	...
	pr_debug("ssi8240: ceainfo: 0x%X, upstatus:0x%X\n",
						ceainfo, upstatus);
...
static irqreturn_t sii8240_irq_thread(int irq, void *data)
{
	...
	if (ret < 0) {
		pr_err("[ERROR] %s() DISC_INTR_REG\n", __func__);
		goto err_exit2;
	}
	ret = mhl_read_byte_reg(disc, DISC_INTR_ENABLE_REG, &intr1_en);
	if (ret < 0) {
		pr_err("[ERROR] %s() DISC_INTR_ENABLE_REG\n", __func__);
		goto err_exit2;
	}
	ret = mhl_read_byte_reg(cbus, CBUS_CONN_STATUS_REG, &cbus_con_st);
	if (ret < 0) {
		pr_err("[ERROR] %s() CBUS_CONN_STATUS_REG\n", __func__);
		goto err_exit2;
	}
	...
	pr_debug("sii8240: state:%d disc:0x%X/0x%X, cbus_con:0x%X ",
		sii8240->state,	intr1, intr1_en, cbus_con_st);
	pr_debug(" tmds:0x%X,0x%X cbus:0x%X,0x%X,0x%X,0x%X\n",
		int1_status, int2_status, cbus1, cbus2, cbus3, cbus4);
	...
	case STATE_MHL_DISCOVERY_SUCCESS:
		...
		/*checking of MHL link*/

		/*Upstream TMDS interrupt register*/
		if (int2_status & BIT_INTR5_CKDT_CHANGE) {
			pr_info("sii8240: CKDT: Upstream TMDS interrupt register\n");
#ifdef SFEATURE_UNSTABLE_SOURCE_WA
			if (sii8240->hdmi_sink)
		...
		if (int2_status & BIT_INTR5_SCDT_CHANGE) {
			...
			if (BIT_TMDS_CSTAT_P3_SCDT & value) {
				pr_info("sii8240: BIT_TMDS_CSTAT_P3_SCDT\n");
				...
				pr_info("sii8240: disable_encryption\n");
		}
...
err_exit2:
	...
	pr_debug("sii8240: IRQ_HANDLED\n");
	return IRQ_HANDLED;
}

#if defined(CONFIG_SS_FACTORY) || defined(CONFIG_MHL_SWING_LEVEL)
static struct sii8240_data *sii8240_factory;
#endif
...
static ssize_t sii8240_swing_test_show(struct class *dev,
		struct class_attribute *attr, char *buf)
{
	u8 clk = (sii8240_factory->pdata->swing_level >> 3) & 0x07;
	u8 data = sii8240_factory->pdata->swing_level & 0x07;

	return sprintf(buf, "SII8240,0x%X,0x%X,0x%X\n", clk, data,
			sii8240_factory->pdata->swing_level);
		...
		const char *buf, size_t size)
{
	if (buf[0] >= '0' && buf[0] <= '7' &&
			buf[1] >= '0' && buf[1] <= '7')
		sii8240_factory->pdata->swing_level = ((buf[0] - '0') << 3) |
			(buf[1] - '0');
	else
		sii8240_factory->pdata->swing_level = 0x2D; /*Clk=5 and Data=5*/
...
#define SII_ID 0x82
static ssize_t sii8240_test_show(struct class *class,
					struct class_attribute *attr,
					char *buf)
{
	struct i2c_client *tmds = sii8240_factory->pdata->tmds_client;
	int size;
	u8 sii_id = 0;

	sii8240_factory->pdata->hw_onoff(1);
	sii8240_factory->pdata->hw_reset();

	mhl_read_byte_reg(tmds, 0x03, &sii_id);
	pr_info("sii8240: check mhl : %X\n", sii_id);

	sii8240_factory->pdata->hw_onoff(0);

	size = sprintf(buf, "%d\n", sii_id == SII_ID ? 1 : 0);
	return size;
}
static CLASS_ATTR(test_result, 0664,
			sii8240_test_show, NULL);
#endif

#ifdef CONFIG_MUIC_NOTIFIER
static void sii8240_notifier_delay_work(struct work_struct *work)
{
	struct sii8240_data *sii8240 = container_of(work, struct sii8240_data,
						notifier_register_work.work);

	muic_notifier_register(&sii8240->mhl_nb,
			sii8240_detection_callback, MUIC_NOTIFY_DEV_MHL);
}
#endif /* CONFIG_MUIC_NOTIFIER */
			...
			const struct i2c_device_id *id)
{
	...
	if (strcmp(client->adapter->name, "exynos5250-i2c")) {
...
#ifdef CONFIG_MUIC_NOTIFIER
	INIT_DELAYED_WORK(&sii8240->notifier_register_work, sii8240_notifier_delay_work);
#endif

#ifdef SII8240_CHECK_MONITOR
	INIT_WORK(&sii8240->mhl_link_monitor_work, sii8240_link_monitor_work);
	setup_timer(&sii8240->mhl_timer, sii8240_link_monitor_timer,
		 (unsigned long)sii8240);
#endif
	...
	ret = request_threaded_irq(client->irq, NULL, sii8240_irq_thread,
			...
			"sii8240", sii8240);
	if (ret < 0) {
		dev_err(&client->dev, "[MHL]request irq Q failing\n");
		goto err_exit1;
	}
	disable_irq(client->irq);
	sii8240->irq_enabled = false;

	sii8240->hdcp_support = true;
	/*MHL sysfs entry*/
...
#endif
#ifdef CONFIG_MHL_SWING_LEVEL
	if (sii8240_factory->pdata->swing_level == 0)
		sii8240_factory->pdata->swing_level = 0x2D;
...
{
	int ret;
	pr_info("[MHL] sii8240_init was called");
#ifdef CONFIG_OF
