...
struct devfreq_clk_info aclk_gscl_333[] = {
	...
	{LV2,	211000000,	0,	NULL},
	{LV3,	211000000,	0,	NULL},
	{LV4,	159000000,	0,	NULL},
...
int district_level_by_disp_333[] = {
	LV2,
	LV4,
#if defined(CONFIG_EXYNOS5430_HD) || defined(CONFIG_EXYNOS5430_FHD)
	LV5,
	LV6,
#elif defined(CONFIG_EXYNOS5430_WQHD)
	LV5,
					...
					int old_idx)
{
	...
	sec_debug_aux_log(SEC_DEBUG_AUXLOG_CPU_BUS_CLOCK_CHANGE,
		"[INT] %7d <= %7d",
		devfreq_int_opp_list[target_idx].freq, devfreq_int_opp_list[old_idx].freq);
					...
					u32 flags)
{
	...
	*target_freq = opp_get_freq(target_opp);
	...
	target_volt_m = data_int->volt_constraint_isp;
	if (target_volt_m < data_int->target_volt)
		target_volt_m = data_int->target_volt;
	if (target_volt_m < ISP_CONSTRAINT_VOLT_BASE)
		target_volt_m = ISP_CONSTRAINT_VOLT_BASE;
	target_volt_m += 50000;
	...
	if (old_freq < *target_freq) {
		...
		regulator_set_voltage(int_data->vdd_int_m, target_volt_m, target_volt_m + VOLT_STEP);
...
static int exynos5_devfreq_int_probe(struct platform_device *pdev)
{
	...
	index = exynos5_devfreq_int_get_idx(devfreq_int_opp_list,
                                                ...
                                                freq);
        if (index < 0) {
                pr_err("DEVFREQ(INT) : Failed to find index abb\n");
                ret = -EINVAL;
                goto err_opp;
        }
