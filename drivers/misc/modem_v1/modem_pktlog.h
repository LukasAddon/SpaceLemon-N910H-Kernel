...
#define pktlog_tx_bottom_skb(sl, skb)	pktlog_queue_skb((sl)->pktlog, (PKT_TX | PKT_BOT) , skb)
#define pktlog_tx_top_skb(sl, skb)	pktlog_queue_skb((sl)->pktlog, (PKT_TX | PKT_TOP) , skb)
#define pktlog_rx_bottom_skb(sl, skb)	pktlog_queue_skb((sl)->pktlog, (PKT_RX | PKT_BOT) , skb)
#define pktlog_rx_top_skb(sl, skb)	pktlog_queue_skb((sl)->pktlog, (PKT_RX | PKT_TOP) , skb)
#define pktlog_text_buf(sl, buf)	do {} while(0)

#else /* else of DEBUG_PKTLOG */
inline static struct pktlog_data *create_pktlog(char *name) { return NULL; }
#define remove_pktlog(pkt)		do {} while(0)
#define pktlog_tx_bottom_skb(sl, skb)	do {} while(0)
#define pktlog_tx_top_skb(sl, skb)	do {} while(0)
#define pktlog_rx_bottom_skb(sl, skb)	do {} while(0)
#define pktlog_rx_top_skb(sl, skb)	do {} while(0)
#define pktlog_text_buf(sl, buf)	do {} while(0)
