			...
			void (*cp_fail_fn)(struct modem_link_pm *))
{
	...
	*/
	...
	/*
	Register call-back functions which will be invoked by the corresponding
	physical link medium driver after the link is unmounted, suspended,
	resumed, and mounted respectively
	*/
	pm_svc_set_owner(pm_svc, pm);
	pm_svc_import_lock(pm_svc, &pm->lock);
	pm_svc->dev_pm_ops = true;
	pm_svc->reset_cb = link_reset_cb;
	pm_svc->unmount_cb = link_unmount_cb;
	pm_svc->suspend_cb = link_suspend_cb;
	pm_svc->resume_cb = link_resume_cb;
	pm_svc->mount_cb = link_mount_cb;
	pm_svc->error_cb = link_error_cb;

	/*
	Register PM service provided by the corresponding physical link driver
	*/
	pm->pm_svc = pm_svc;
...
int link_pm_parse_dt_gpio_pdata(struct device_node *np, struct modem_data *mdm)
{
	...
	mdm->gpio_ap_wakeup = of_get_named_gpio(np, "gpio_cp2ap_wakeup", 0);
	...
	mdm->gpio_cp_wakeup = of_get_named_gpio(np, "gpio_ap2cp_wakeup", 0);
	...
	mdm->gpio_cp_status = of_get_named_gpio(np, "gpio_cp2ap_status", 0);
	...
	mdm->gpio_ap_status = of_get_named_gpio(np, "gpio_ap2cp_status", 0);
