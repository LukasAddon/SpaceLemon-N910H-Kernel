...
static int usb_get_rpm_status(struct usb_device *udev)
{
#ifdef CONFIG_WAIT_CP_SUSPEND_READY
	struct link_pm_data *pmdata = dev_get_drvdata(&udev->dev);

	if (pmdata && pmdata->check_cp_suspend_ready) {
		if (pmdata->check_cp_suspend_ready(pmdata))
			return RPM_SUSPENDING;
	}
#endif
	return udev->dev.power.runtime_status;
}
#else
static inline int usb_get_rpm_status(struct usb_device *dev) { return 0; }
#endif
...
static void skb_reset_buff_pointer(struct sk_buff *skb)
{
...
#if defined(CONFIG_SEC_MODEM_XMM6360) || defined(CONFIG_SEC_MODEM_M74XX)
static void mif_net_suspend(struct if_usb_devdata *pipe_data, int flag)
{
	if (!pipe_data->iod->msd || !flag)
		return;

	if (!pipe_data->net_suspend)
		netif_tx_flowctl(pipe_data->iod->msd, true);
...
static void mif_net_resume(struct if_usb_devdata *pipe_data, int flag)
{
	...
	if (pipe_data->net_suspend) {
		...
		if (!pipe_data->net_suspend)
			netif_tx_flowctl(pipe_data->iod->msd, false);
	}
	return;
}
#else
...
static int usb_init_communication(struct link_device *ld, struct io_device *iod)
{
	...
	if (!usb_ld->if_usb_connected) {
		...
		return -ENODEV;
	}  else if (ld->com_state != COM_ONLINE) {
		mif_err("com_state=%d, open fail\n", ld->com_state);
		return -ENODEV;
	}
...
int usb_tx_skb(struct if_usb_devdata *pipe_data, struct sk_buff *skb)
{
	...
	if (usb_get_rpm_status(usb_ld->usbdev) != RPM_ACTIVE) {
	...
	if (!(pipe_data->info->flags & FLAG_SEND_NZLP || skbpriv(skb)->nzlp))
	...
	if (pipe_data->state == STATE_SUSPENDED ||
		usb_get_rpm_status(usb_ld->usbdev) == RPM_SUSPENDING) {
		usb_anchor_urb(urb, &pipe_data->tx_deferd_urbs);
...
#ifdef CONFIG_WAIT_CP_SUSPEND_READY
	struct link_pm_data *pmdata = dev_get_drvdata(&usb_ld->usbdev->dev);
	if (pmdata && pmdata->wait_cp_suspend_ready)
		if (pmdata->wait_cp_suspend_ready(pmdata, pipe_data->usbdev) < 0)
			return -EBUSY;
#endif
...
static void if_usb_disconnect(struct usb_interface *intf)
{
	...
	if (pipe_data->idx == 6 /* last channel */
		&& pipe_data->usb_ld->ld.mc->phone_state == STATE_ONLINE) {
		pipe_data->usb_ld->events = 0;
		set_bit(LINK_EVENT_RECOVERY, &pipe_data->usb_ld->events);
		schedule_delayed_work(&pipe_data->usb_ld->link_event, HZ);
		mif_info("trigger CP recovery work\n");
	}
	return;
}
	...
	struct usb_interface *intf)
{
	...
	if (pipe_data->info->flags & FLAG_BOOT_DOWN)
		skb_queue_purge(&pipe_data->iod->sk_rx_q);
...
#if defined(CONFIG_OF) && \
	(defined(CONFIG_UMTS_MODEM_XMM6360) || defined(CONFIG_LTE_MODEM_XMM7260))
		...
		unsigned long events, void *arg)
{
	...
	struct modem_ctl *mc = usb_ld->ld.mc;
	unsigned val = gpio_get_value(mc->gpio_cp_reset);
...
static struct usb_id_info ste_boot_down_info = {
	...
	.flags = FLAG_BOOT_DOWN | FLAG_SEND_NZLP,
	.bind = cdc_acm_bind,
	.unbind = cdc_acm_unbind,
};
...
static struct usb_device_id if_usb_ids[] = {
	...
	{ USB_DEVICE_AND_INTERFACE_INFO(0x04cc, 0x0500, USB_CLASS_VENDOR_SPEC,
		0x01, USB_CDC_ACM_PROTO_VENDOR),
...
#if defined(CONFIG_OF) && \
	(defined(CONFIG_UMTS_MODEM_XMM6360) || defined(CONFIG_LTE_MODEM_XMM7260))
	/* STE & BRCM didn't need below notifier */
	usb_ld->phy_nfb.notifier_call = mif_usb2phy_notify;
	phy_register_notifier(&usb_ld->phy_nfb);
#endif
