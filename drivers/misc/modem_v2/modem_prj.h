...
struct sipc_ops {
	...
	u32 (*multifmt_length)
		(struct io_device *, struct sipc_hdr *, size_t, size_t);
	int (*recv_demux)
...
struct io_device {
	...
	struct sk_buff_head sk_multi_q[SIPC_MULTIFMT_ID_MAX+1];
	/* maximum size of each multi framming packet */
	unsigned multi_len;
...
struct modem_ctl {
	...
	/* For Shannon/Ericsson hsic modem to use cp crash ram dump */
	struct workqueue_struct *cp_crash_dump_wq;
	struct delayed_work cp_crash_dump_dwork;

	struct notifier_block cp_crash_nfb;
};

enum cp_crash_state_notify {
	CP_FORCE_CRASH_EVENT,
};
