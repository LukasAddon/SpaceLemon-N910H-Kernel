						...
						const char *buf, size_t count)
{
	...
	case 0:
		if (rear_flash_status != 1) {
			pinctrl = devm_pinctrl_get_select(global_dev, FLED_PINCTRL_STATE_DEFAULT);
			if (IS_ERR(pinctrl))
		...
		pinctrl = devm_pinctrl_get_select(global_dev, FLED_PINCTRL_STATE_SLEEP);
		if (IS_ERR(pinctrl))
	...
	case 1:
		...
		pr_info("Torch HIGH : led_irq_gpio1(%d)_irq_gpio2(%d)\n", led_irq_gpio1, led_irq_gpio2);
#if defined(CONFIG_FLED_RT5033)
		if (fled_info)
		...
		break;
	case 100:
		pr_info("Torch Factory : led_irq_gpio1(%d)_irq_gpio2(%d)\n", led_irq_gpio1, led_irq_gpio2);
		pinctrl = devm_pinctrl_get_select(global_dev, FLED_PINCTRL_STATE_DEFAULT);
		if (IS_ERR(pinctrl))
			pr_err("%s: flash %s pins are not configured\n", __func__, FLED_PINCTRL_STATE_DEFAULT);
		else
...
int create_flash_sysfs(void)
{
	...
	pr_info("flash_sysfs: sysfs test!!!! (%s)\n",__func__);
...
static int rt5033_fled_probe(struct platform_device *pdev)
{
	...
	struct rt5033_fled_platform_data *pdata;
	rt5033_fled_info_t *fled_info;
	pr_info("%s : Richtek RT5033 FlashLED driver probing...\n", __func__);
#ifdef CONFIG_OF
