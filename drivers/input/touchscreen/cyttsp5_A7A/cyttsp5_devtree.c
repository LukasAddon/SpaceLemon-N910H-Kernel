		...
		struct device_node *core_node)
{
	...
	pdata->irq_gpio = of_get_named_gpio(core_node, "cy,irq_gpio", 0);
		if (gpio_is_valid(pdata->irq_gpio)) {
			val = gpio_request_one(pdata->irq_gpio, GPIOF_DIR_IN, "cy,tsp_int");
			if (val) {
				pr_info( "Unable to request tsp_int [%d]\n", pdata->irq_gpio);
				goto fail;
			}
		} else {
			pr_info( "Failed to get irq gpio\n");
			goto fail;
		}
	...
	if (of_property_read_string(core_node, "cy,regulator_vdd", &pdata->regulator_vdd)){
			pr_info("[wisdyd] failed to get regulrator_vdd name property \n" );
			goto fail_free;
	}

	if (of_property_read_string(core_node, "cy,regulator_avdd", &pdata->regulator_avdd)){
			pr_info("[wisdyd] failed to get regulrator_avdd name property \n" );
			goto fail_free;
	}
