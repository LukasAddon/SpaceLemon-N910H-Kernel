...
#error CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_BINARY_FW_UPGRADE defined
#endif
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_PLATFORM_TTCONFIG_UPGRADE
#error CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_PLATFORM_TTCONFIG_UPGRADE defined
#endif
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_MANUAL_TTCONFIG_UPGRADE
#error CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_MANUAL_TTCONFIG_UPGRADE defined
#endif*/
		...
		u32 fw_ver_new, u32 fw_revctrl_new)
{
	...
	if (0) {
		dev_dbg(dev, "%s: block update, will not upgrade\n",
				__func__);
		return -1;
	}
...
static int upgrade_firmware_from_class(struct device *dev)
{
	...
	retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_NOHOTPLUG, "",
			dev, GFP_KERNEL, dev, _cyttsp5_firmware_cont);
	if (retval < 0) {
		...
		u16 row_number, u16 row_size, u8 *data)
{
	...
	rc = cmd->cmd->write_conf_block(dev, 0, row_number,
		...
		const u8 *ttconfig_data, int ttconfig_size)
{
	...
	rc = cmd->cmd->verify_config_block_crc(dev, 0, ebid,
		...
		const u8 *ttconfig_data, int ttconfig_size)
{
	...
	rc = cyttsp5_get_ttconfig_crc(dev, ttconfig_data, ttconfig_size,
			&cfg_crc_new);
	if (rc)
		return 0;
