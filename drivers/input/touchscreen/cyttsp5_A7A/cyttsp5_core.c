...
#define CYTTSP5_REPORT_DATA_FROM_TABLE
...
#ifdef RTD_TUNER_SUPPORT
static int rtd_print(struct cyttsp5_core_data *cd, u8 *buf, int buf_len,
		const u8 *data_name)
{
	int len = strlen(data_name);
	int i, n;
	...
	mutex_lock(&cd->rtd_lock);
	if (!cd->rtd_buf)
		goto exit;

	if (cd->rtd_buf_len + (len + buf_len) > CY_MAX_PRBUF_SIZE)
		goto exit;

	if (len + buf_len == 0)
		goto exit;

	remain = CY_MAX_PRBUF_SIZE - cd->rtd_buf_len;
	if (remain < len)
		len = remain;

	p = cd->rtd_buf + cd->rtd_buf_len;
	memcpy(p, data_name, len);
	cd->rtd_buf_len += len;
	p += len;
	remain -= len;
	...
	*p = 0;
	...
	n = scnprintf(p, remain, "\n");
	if (!n)
		cd->rtd_buf[cd->rtd_buf_len] = 0;
	cd->rtd_buf_len += n;
	wake_up(&cd->wait_q);
exit:
	mutex_unlock(&cd->rtd_lock);
	return 0;
}

#endif

#ifndef CYTTSP5_REPORT_DATA_FROM_TABLE
...
#endif//CYTTSP5_REPORT_DATA_FROM_TABLE
...
#define CAL_UNKNOWN (-1)
#define CAL_DONE 1
#define CAL_NOT_DONE 0
static const u8 cyttsp5_security_key[] = {
	0xA5, 0x01, 0x02, 0x03, 0xFF, 0xFE, 0xFD, 0x5A
};
#define USER_DATA_BLOCK_MAX_SIZE 62
#define USER_DATA_BUF_SIZE (USER_DATA_BLOCK_MAX_SIZE + 2)

static int cyttsp5_read_full_ddata(struct device *dev,
	u8* read_buf)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	u16 crc;
...
static int cyttsp5_write_cal_flag_in_user_data(struct device *dev)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	u8 read_buf[USER_DATA_BUF_SIZE];
	u16 crc;
...
static int cyttsp5_hid_output_bl_launch_app_fast_(struct cyttsp5_core_data *cd)
{
	struct cyttsp5_hid_output hid_output = {
		HID_OUTPUT_BL_COMMAND(HID_OUTPUT_BL_LAUNCH_APP_FAST),
		.novalidate = 1,
	};
...
static int cyttsp5_pinctrl_configure(struct cyttsp5_core_data *cd, bool active)
{
	...
	if (active) {
		set_state_tsp =
			pinctrl_lookup_state(cd->pinctrl,
						"on_state");
		if (IS_ERR(set_state_tsp)) {
			pr_err("%s: cannot get pinctrl(tsp_en_gpio) active state\n", __func__);
		...
		set_state_tsp =
			pinctrl_lookup_state(cd->pinctrl,
						"off_state");
		if (IS_ERR(set_state_tsp)) {
			pr_err("%s: cannot get pinctrl(tsp_en_gpio) sleep state\n", __func__);
			return PTR_ERR(set_state_tsp);
		}
	}

	retval = pinctrl_select_state(cd->pinctrl, set_state_tsp);
	if (retval) {
		pr_err("%s: cannot set pinctrl %s state\n",
				__func__, active ? "active" : "suspend");
		return retval;
	}
...
#ifdef CYTTSP5_REPORT_DATA_FROM_TABLE
static const struct cyttsp5_tch_abs_params table_tch_abs[CY_TCH_NUM_ABS] = {
/*         ofs size min  max    bofs  report */
/* X   */ {2,  2,   0,   65536, 0,    1},
/* Y   */ {4,  2,   0,   65536, 0,    1},
/* P   */ {6,  1,   0,   256,   0,    1},
/* T   */ {1,  1,   0,   32,    0,    1},
...
static const struct cyttsp5_tch_abs_params table_tch_hdr[CY_TCH_NUM_HDR] = {
/*                      ofs size min  max    bofs  report */
/* SCAN TIME         */ {0, 2,   0,   65536, 0,    1},
/* NUMBER OF RECORDS */ {2, 1,   0,   32,    0,    1},
/* LARGE OBJECT      */ {2, 1,   0,   2,     5,    1},
/* NOISE EFFECT      */ {3, 1,   0,   8,     0,    1},
/* REPORT_COUNTER    */ {3, 1,   0,   4,     6,    1},
};

static const struct cyttsp5_report_desc_data table_report_desc_data = {
/* tch_report_id   */1,
/* tch_record_size */10,
/* tch_header_size */7,
/* btn_report_id   */3
...
static void cyttsp5_fill_report_data_from_table(struct cyttsp5_core_data *cd)
{
	struct cyttsp5_sysinfo *si = &cd->sysinfo;
	int i;

	for (i = CY_TCH_X; i < CY_TCH_NUM_ABS; i++) {
		si->tch_abs[i] = table_tch_abs[i];
	}

	for (i = CY_TCH_TIME; i < CY_TCH_NUM_HDR; i++) {
		si->tch_hdr[i] = table_tch_hdr[i];
	}

	si->desc = table_report_desc_data;
}
#else//CYTTSP5_REPORT_DATA_FROM_TABLE
/* Fills si->tch_hdr, si->tch_abs, si->desc */
...
static void cyttsp5_ta_cb(struct tsp_callbacks *cb, int ta_status)
{
	struct cyttsp5_core_data *cd =
	...
	cd->ta_status = ta_status;
}

static void cyttsp5_register_callback(struct tsp_callbacks *cb)
{
...
static int cyttsp5_core_sleep_(struct cyttsp5_core_data *cd)
{
	...
	cyttsp5_pinctrl_configure(cd, false);
#if CYTTSP5_USE_SLEEP
...
static int move_real_time_debug_data(struct cyttsp5_core_data *cd,
	struct cyttsp5_sysinfo *si)
{
#ifdef RTD_TUNER_SUPPORT
	int size = get_unaligned_le16(&cd->input_buf[0]);
	if (size)
		rtd_print(cd, cd->input_buf, size, "REAL_TIME=");
#endif
	...
	struct cyttsp5_sysinfo *si)
{
	...
	if (unlikely(num_cur_tch > MAX_TOUCH_ID_NUMBER))
...
static int parse_touch_input(struct cyttsp5_core_data *cd, int size)
{
	...
	else if (report_id == HID_TTHE_DEBUG_REPORT_ID)
		rc = move_real_time_debug_data(cd, si);
...
static int cyttsp5_parse_input(struct cyttsp5_core_data *cd)
{
	...
	cyttsp5_pr_buf(cd->dev, cd->pr_buf, cd->input_buf, 2 + size,
			"irq_read");
	...
	if (size == 0) {
		...
		if (!cd->hid_reset_cmd_state && // set in reset() and cleared here.
			...
			if (cd->hid_cmd_state == HID_OUTPUT_START_BOOTLOADER + 1 ||
				...
				cd->hid_cmd_state == HID_OUTPUT_BL_LAUNCH_APP_FAST + 1 ||
				cd->hid_cmd_state == HID_OUTPUT_USER_CMD + 1)
				cd->hid_cmd_state = 0;
			wake_up(&cd->wait_q);
		}
	...
	if (report_id != cd->sysinfo.desc.tch_report_id
			...
			&& report_id != HID_TTHE_DEBUG_REPORT_ID)
		is_command = 1;
...
_cyttsp5_request_upgrade_firmware_from_platform(struct device *dev)
{
	...
	return cd->upgrade_firmware_from_platform;
}
...
exit_reset:
	...
	if (cd->mode == CY_MODE_BOOTLOADER) {
		...
		rc = cyttsp5_hid_output_bl_launch_app_fast_(cd);
		if (rc < 0) {
...
#ifdef CYTTSP5_REPORT_DATA_FROM_TABLE
	cyttsp5_fill_report_data_from_table(cd);
#else
	...
	if (!cd->sysinfo.ready || cd->check_postl) {
#endif
		cd->check_postl = 0;
		rc = cyttsp5_hid_output_get_sysinfo_(cd);
		if (rc) {
...
static int cyttsp5_startup(struct cyttsp5_core_data *cd)
{
	...
	cyttsp5_samsung_factory_startup_attention(cd->dev);
...
#ifdef RTD_TUNER_SUPPORT
static int rtd_debugfs_open(struct inode *inode, struct file *filp)
{
	struct cyttsp5_core_data *cd = inode->i_private;
	filp->private_data = inode->i_private;
	...
	cd->rtd_buf = kzalloc(CY_MAX_PRBUF_SIZE, GFP_KERNEL);
	if (!cd->rtd_buf)
		return -ENOMEM;
...
static int rtd_debugfs_close(struct inode *inode, struct file *filp)
{
	struct cyttsp5_core_data *cd = filp->private_data;
	filp->private_data = NULL;
...
static ssize_t rtd_debugfs_read(struct file *filp, char __user *buf,
		size_t count, loff_t *ppos)
{
	struct cyttsp5_core_data *cd = filp->private_data;
	int size;
	...
	wait_event_interruptible(cd->wait_q,
			cd->rtd_buf_len != 0 || cd->rtd_exit);
	mutex_lock(&cd->rtd_lock);
	if (cd->rtd_exit) {
		mutex_unlock(&cd->rtd_lock);
		return 0;
	}
	if (count > cd->rtd_buf_len)
		size = cd->rtd_buf_len;
	else
		size = count;
	if (!size) {
	...
	ret = copy_to_user(buf, cd->rtd_buf, cd->rtd_buf_len);
	if (ret == size)
		return -EFAULT;
	size -= ret;
	cd->rtd_buf_len -= size;
...
static const struct file_operations rtd_debugfs_fops = {
	.open = rtd_debugfs_open,
	.release = rtd_debugfs_close,
	.read = rtd_debugfs_read,
};
#endif
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	cd->pinctrl = devm_pinctrl_get(cd->dev);
...
#ifdef RTD_TUNER_SUPPORT
	mutex_init(&cd->rtd_lock);
	cd->rtd_debugfs = debugfs_create_file(CYTTSP5_RTD_TUNER_FILE_NAME,
			0644, NULL, cd, &rtd_debugfs_fops);
#endif

	device_init_wakeup(dev, 1);

	cd->cal_flag = CAL_UNKNOWN;
...
#ifdef USE_TSP_TA_CALLBACKS
	cd->register_cb = cyttsp5_register_callback;
	cd->callbacks.inform_charger = cyttsp5_ta_cb;
	if (cd->register_cb)
...
int cyttsp5_release(struct cyttsp5_core_data *cd)
{
	...
	wake_up(&cd->wait_q);
	mutex_unlock(&cd->rtd_lock);
	debugfs_remove(cd->rtd_debugfs);
#endif

#ifdef USE_TSP_TA_CALLBACKS
	cd->callbacks.inform_charger = NULL;
	cd->register_cb = NULL;
