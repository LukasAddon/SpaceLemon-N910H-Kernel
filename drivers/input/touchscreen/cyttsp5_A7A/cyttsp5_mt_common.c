...
// Palm swipe workaround for A7 FW 5.0+
#define SCALE_MAJ_MIN_FOR_FINGER
		...
		u16 sumsize, bool palm)
{
	...
	for (t = 0; t < max_slots; t++) {
		if (test_bit(t, ids)) {
			...
			input_report_abs(input, ABS_MT_GRIP, sumsize);
...
static void cyttsp5_mt_lift_all(struct cyttsp5_mt_data *md)
{
	...
	if (md->touch_pressed_num != 0) {
		dev_info(md->dev, "%s force dvfs off\n", __func__);
		...
		struct cyttsp5_touch *tch, struct cyttsp5_mt_data *md, int t)
{
	...
	if ((tch->abs[CY_TCH_E] == CY_EV_TOUCHDOWN) &&
		...
		(tch->abs[CY_TCH_O] != CY_OBJ_HOVER))
		dev_info(dev, "R [%d] x=%d y=%d z=%d M=%d m=%d ver=%02x\n",
			t, tch->abs[CY_TCH_X],
			tch->abs[CY_TCH_Y],
			tch->abs[CY_TCH_P],
			tch->abs[CY_TCH_MAJ],
...
#ifdef SCALE_MAJ_MIN_FOR_FINGER
static void inline scale_maj_min(struct cyttsp5_mt_data *md,
	enum cyttsp5_object_id obj_id, int sig, int *value)
{
	if (md->fw_ver_ic < 0x05)
		return;

	if (obj_id != CY_OBJ_STANDARD_FINGER)
		return;
...
#endif//SCALE_MAJ_MIN_FOR_FINGER
		...
		struct cyttsp5_touch *tch, int num_cur_tch)
{
	...
	for (i = 0; i < num_cur_tch; i++) {
		...
		if (tch->abs[CY_TCH_T] < ABS_PARAM(CY_ABS_ID_OST, CY_MIN_OST) ||
			...
			dev_info(dev, "%s: tch=%d -> bad trk_id=%d max_id=%d\n",
...
#ifdef SCALE_MAJ_MIN_FOR_FINGER
			scale_maj_min(md, tch->abs[CY_TCH_O], sig, &value);
#endif
			input_report_abs(md->input, sig, value);
...
static int cyttsp5_setup_input_device(struct device *dev)
{
	...
	dev_dbg(dev, "%s: register signal=%02X min=%d max=%d\n",
				__func__, signal, min, max);
	input_set_abs_params(md->input, signal = ABS_MT_GRIP,
		min = 0, max = 255, 0, 0);
	dev_dbg(dev, "%s: register signal=%02X min=%d max=%d\n",
				__func__, signal, min, max);
#endif
