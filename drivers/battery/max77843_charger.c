...
static bool max77843_charger_unlock(struct max77843_charger_data *charger)
{
	...
	do {
		max77843_read_reg(i2c, MAX77843_CHG_REG_CHG_CNFG_06, &reg_data);
		chgprot = ((reg_data & 0x0C) >> 2);
		if (chgprot != 0x03) {
			pr_err("%s: unlock err, chgprot(0x%x), retry(%d)\n",
					__func__, chgprot, retry_cnt);
			max77843_write_reg(i2c, MAX77843_CHG_REG_CHG_CNFG_06,
					   (0x03 << 2));
			need_init = true;
			msleep(20);
		} else {
...
static int max77843_get_vbus_state(struct max77843_charger_data *charger)
{
	...
	max77843_read_reg(charger->i2c,
			  MAX77843_CHG_REG_CHG_DTLS_00, &reg_data);
...
static int max77843_get_charging_health(struct max77843_charger_data *charger)
{
	...
	if (state == POWER_SUPPLY_HEALTH_GOOD) {
		...
		max77843_read_reg(charger->i2c,
				  MAX77843_CHG_REG_CHG_DTLS_01, &chg_dtls);
		chg_dtls = ((chg_dtls & MAX77843_CHG_DTLS) >>
			    MAX77843_CHG_DTLS_SHIFT);
		max77843_read_reg(charger->i2c,
				  MAX77843_CHG_REG_CHG_CNFG_00, &chg_cnfg_00);

		/* print the log at the abnormal case */
		if((charger->is_charging == 1) && (chg_dtls & 0x08)) {
			max77843_read_reg(charger->i2c,
				MAX77843_CHG_REG_CHG_DTLS_00, &chg_dtls_00);
			max77843_read_reg(charger->i2c,
				MAX77843_CHG_REG_CHG_CNFG_01, &chg_cnfg_01);
			max77843_read_reg(charger->i2c,
				MAX77843_CHG_REG_CHG_CNFG_02, &chg_cnfg_02);
			max77843_read_reg(charger->i2c,
				MAX77843_CHG_REG_CHG_CNFG_04, &chg_cnfg_04);
			max77843_read_reg(charger->i2c,
					  MAX77843_CHG_REG_CHG_CNFG_09, &chg_cnfg_09);
			max77843_read_reg(charger->i2c,
					  MAX77843_CHG_REG_CHG_CNFG_12, &chg_cnfg_12);
...
static int max77843_get_input_current(struct max77843_charger_data *charger)
{
	...
	if (charger->cable_type == POWER_SUPPLY_TYPE_WIRELESS) {
		max77843_read_reg(charger->i2c,
				  MAX77843_CHG_REG_CHG_CNFG_10, &reg_data);
		...
		max77843_read_reg(charger->i2c,
				  MAX77843_CHG_REG_CHG_CNFG_09, &reg_data);
...
static bool max77843_check_battery(struct max77843_charger_data *charger)
{
	...
	max77843_read_reg(charger->i2c,
			  MAX77843_CHG_REG_CHG_INT_OK, &reg_data);

	pr_info("%s : CHG_INT_OK(0x%x)\n", __func__, reg_data);

	max77843_read_reg(charger->i2c,
			  MAX77843_CHG_REG_CHG_DTLS_00, &reg_data2);
				       ...
				       int input_current)
{
	...
	if (charger->cable_type == POWER_SUPPLY_TYPE_WIRELESS) {
		set_reg = MAX77843_CHG_REG_CHG_CNFG_10;
		max77843_read_reg(charger->i2c,
				  set_reg, &reg_data);
		reg_data &= ~MAX77843_CHG_WCIN_LIM;
	} else {
		set_reg = MAX77843_CHG_REG_CHG_CNFG_09;
		max77843_read_reg(charger->i2c,
				  set_reg, &reg_data);
	...
	int enable)
{
	...
	max77843_read_reg(charger->i2c, MAX77843_CHG_REG_CHG_CNFG_00, &reg_data);

	if (enable)
		reg_data |= MAX77843_MODE_CHGR;
	else
		reg_data &= ~MAX77843_MODE_CHGR;

	pr_debug("%s : CHG_CNFG_00(0x%02x)\n", __func__, reg_data);

	max77843_write_reg(charger->i2c,MAX77843_CHG_REG_CHG_CNFG_00, reg_data);
}
				...
				struct max77843_charger_data *charger)
{
	...
	if (((charger->cable_type == POWER_SUPPLY_TYPE_HV_MAINS) ||
		...
		max77843_write_reg(charger->i2c, MAX77843_CHG_REG_CHG_CNFG_01,
				   chg_cnfg_01);
...
static void max77843_charger_initialize(struct max77843_charger_data *charger)
{
	...
	max77843_write_reg(charger->i2c, MAX77843_CHG_REG_CHG_INT_MASK, 0x9A);

	/* unlock charger setting protect */
	reg_data = (0x03 << 2);
	max77843_write_reg(charger->i2c, MAX77843_CHG_REG_CHG_CNFG_06, reg_data);
	 ...
	 */
	reg_data = 0x30;
	max77843_write_reg(charger->i2c, MAX77843_CHG_REG_CHG_CNFG_01, reg_data);

	/*
	 * charge current 466mA(default)
	 * otg current limit 1200mA
	 */
	max77843_read_reg(charger->i2c, MAX77843_CHG_REG_CHG_CNFG_02, &reg_data);
	reg_data |= (1 << 7);
	max77843_write_reg(charger->i2c, MAX77843_CHG_REG_CHG_CNFG_02, reg_data);
	 ...
	 */
	reg_data = 0x36;
	max77843_write_reg(charger->i2c, MAX77843_CHG_REG_CHG_CNFG_03, reg_data);
	...
	max77843_update_reg(charger->i2c, MAX77843_CHG_REG_CHG_CNFG_04,
			      ...
			      union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (max77843_read_reg(charger->i2c,
			MAX77843_CHG_REG_CHG_INT_OK, &reg_data) == 0) {
