...
void fg_check_vf_fullcap_range(struct i2c_client *client)
{
	...
	/* compare with previous capacity */
		if (new_vffullcap >
			(fuelgauge->previous_vffullcap * 110 / 100)) {
			dev_info(&client->dev,
				"%s: [Case 2] previous_vffullcap = 0x%04x, NewVfFullCap = 0x%04x\n",
				__func__, fuelgauge->previous_vffullcap,
				new_vffullcap);

			new_vffullcap =
				(fuelgauge->previous_vffullcap * 110) /
				100;
			...
			(fuelgauge->previous_vffullcap * 90 / 100)) {
			dev_info(&client->dev,
...
static int get_fuelgauge_soc(struct i2c_client *client)
{
	...
	fullcap_check_interval =
		(ts.tv_sec - fuelgauge->fullcap_check_interval);
	...
	if (fuelgauge->low_batt_boot_flag) {
		fg_soc = 0;
		...
		if (fuelgauge->pdata->check_cable_callback &&
			...
			fuelgauge->low_batt_boot_flag = 0;
		}
	...
	if (value.intval != POWER_SUPPLY_STATUS_DISCHARGING &&
		...
		if (fuelgauge->full_check_flag == 2) {
			dev_info(&client->dev,
				"%s: force fully charged SOC !! (%d)",
				__func__, fuelgauge->full_check_flag);
			fg_set_full_charged(client);
			fg_soc = get_fuelgauge_value(client, FG_LEVEL);
		} else if (fuelgauge->full_check_flag < 2)
			dev_info(&client->dev,
				"%s: full_check_flag (%d)",
				__func__, fuelgauge->full_check_flag);

		/* prevent overflow */
		if (fuelgauge->full_check_flag++ > 10000)
			fuelgauge->full_check_flag = 3;
	} else
		fuelgauge->full_check_flag = 0;
...
static void full_comp_work_handler(struct work_struct *work)
{
	struct max77849_fuelgauge_info *fuelgauge =
		container_of(work, struct max77849_fuelgauge_info, full_comp_work.work);
	int avg_current;
	union power_supply_propval value;
...
//	board_fuelgauge_init(fuelgauge);
	...
	if (fuelgauge->pdata->check_cable_callback &&
		...
		POWER_SUPPLY_TYPE_BATTERY) &&
			is_booted_in_low_battery(client))
		fuelgauge->low_batt_boot_flag = 1;
...
//	}
	...
	INIT_DELAYED_WORK(&fuelgauge->full_comp_work,
...
bool fg_full_charged(struct i2c_client *client)
{
	struct max77849_fuelgauge_info *fuelgauge =
				i2c_get_clientdata(client);
	union power_supply_propval value;
				...
				struct max77849_fuelgauge_info *fuelgauge)
{
	...
	/* update capacity_old for max77849_fg_get_atomic_capacity algorithm */
			    ...
			    const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		if (val->intval == POWER_SUPPLY_STATUS_FULL)
			fg_full_charged(fuelgauge->client);
		break;
	case POWER_SUPPLY_PROP_CHARGE_FULL:
		if (val->intval == POWER_SUPPLY_TYPE_BATTERY) {
			if (fuelgauge->pdata->capacity_calculation_type &
				SEC_FUELGAUGE_CAPACITY_TYPE_DYNAMIC_SCALE)
				max77849_fg_calculate_dynamic_scale(fuelgauge);
		}
		break;
...
MODULE_DEVICE_TABLE(of, fuelgague_i2c_match_table);
#endif /* CONFIG_OF */
