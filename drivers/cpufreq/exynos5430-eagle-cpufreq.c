...
#define EMA_VAL_REG		(S5P_VA_CHIPID2 + 0x20)
#define EMA_CON1_SHIFT		2
#define EMA_CON2_SHIFT		5
...
static void __iomem *clk_egl_2_base;
...
static void exynos5430_set_ema_CA15(unsigned int target_volt)
{
	unsigned int tmp;

	tmp = __raw_readl(clk_egl_2_base + CLK_EGL_EMA_OFFSET);
	...
	if (target_volt > EMA_VOLT) {
		if (egl_ema_con1 > 0) {
			...
			__raw_writel(tmp, clk_egl_2_base + CLK_EGL_EMA_OFFSET);
		}
	} else {
		if (egl_ema_con2 > 0) {
			tmp &= ~((EMA_CON_MASK << EGL_L1_EMA_SHIFT) |
					 ...
					 unsigned int new_index)
{
	...
	sec_debug_aux_log(SEC_DEBUG_AUXLOG_CPU_BUS_CLOCK_CHANGE,
		"[A15] %7d <= %7d",
		exynos5430_freq_table_CA15[new_index].frequency, exynos5430_freq_table_CA15[old_index].frequency);
...
static void set_boot_cpu_qos_freq(struct exynos_dvfs_info *info, int lv_idx)
{
	...
	if (lv_idx < L15)
		lv_idx = L15;
#else
	if (lv_idx < L17)                /* booting frequency is 800MHz when factory mode */
		lv_idx = L17;
#endif
	info->boot_cpu_min_qos_timeout = 360 * USEC_PER_SEC;
	info->boot_cpu_max_qos_timeout = 360 * USEC_PER_SEC;
#endif
	info->boot_cpu_min_qos = exynos5430_freq_table_CA15[lv_idx].frequency;
	info->boot_cpu_max_qos = exynos5430_freq_table_CA15[lv_idx].frequency;
}
#endif
...
int __init exynos5_cpufreq_CA15_init(struct exynos_dvfs_info *info)
{
	...
	egl_ema_con1 = exynos5430_ema_valid_check_CA15(egl_ema_con1);
	egl_ema_con2 = exynos5430_ema_valid_check_CA15(egl_ema_con2);

	pr_info("%s: ema_con1[%d], ema_con2[%d]\n", __func__, egl_ema_con1, egl_ema_con2);

	clk_egl_2_base = ioremap(EXYNOS5430_PA_CMU_EGL2, SZ_4K);
	if (!clk_egl_2_base) {
		pr_err("failed to ioremap for EMA\n");
		goto err_ioremap;
	}
	...
	info->set_ema = exynos5430_set_ema_CA15;
